...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	  ...
	  <xsl:for-each select="wp:anchor | wp:inline">
			<xsl:choose>
				<xsl:when test="name(.)='wp:anchor'">
					<xsl:if test="./a:graphic/a:graphicData/pic:pic or ./a:graphic/a:graphicData/wps:wsp">
						<xsl:apply-templates select="a:graphic/a:graphicData/pic:pic|./a:graphic/a:graphicData/wps:wsp">
							<xsl:with-param name="picType" select="'anchor'"/>
							<xsl:with-param name="picFrom" select="$drawingFrom"/>
						</xsl:apply-templates>
					</xsl:if>
					<!-- 组合图形-->
					<xsl:if test="./a:graphic/a:graphicData/wpg:wgp">
						<xsl:apply-templates select="a:graphic/a:graphicData/wpg:wgp">
							<xsl:with-param name="picType" select="'anchor'"/>
							<xsl:with-param name="picFrom" select="$drawingFrom"/>
						</xsl:apply-templates>
					</xsl:if>
      ...
      <图:预定义图形_8018>
        ...
        <图:属性_801D>
          ...
          <xsl:if test="./dsp:spPr/a:ln/a:solidFill//a:alpha">
            <图:透明度_8050>
              <xsl:value-of select="round(100-(./dsp:spPr/a:ln/a:solidFill//a:alpha/@val div 1000))"/>
            </图:透明度_8050>
          </xsl:if>
                  ...
                  <字:句属性_4158>
                    ...
                    <字:是否粗体_4130>
                      <xsl:choose>
                        ...
                        <!--2015-04-26，wudi，去掉not，改条件为(./a:rPr/@b)，start-->
                    ...
                    <字:是否斜体_4131>
                      <xsl:choose>
                        ...
                        <!--2015-04-26，wudi，去掉not，改条件为(./a:rPr/@i)，start-->
  ...
  <!--end-->
	...
	<xsl:template match="wpg:wgp | wpg:grpSp"><!--组合图形转换-->
		<xsl:param name="picFrom"/>
		<xsl:variable name="number">
			<xsl:number format="1" level="any" count="v:rect | wp:anchor | wp:inline | v:shape"/>
		</xsl:variable>
		<xsl:for-each select="wps:wsp|wpg:grpSp">
			<xsl:if test="name(.)='wps:wsp'">
				<xsl:apply-templates select=".">
					<xsl:with-param name="picType" select="'anchor'"/>
					<xsl:with-param name="picFrom" select="'grpsp'"/>
				</xsl:apply-templates>
			</xsl:if>
			<xsl:if test="name(.)='wpg:grpSp'">
				<xsl:apply-templates select=".">
					<xsl:with-param name="picFrom" select="'grpsp'"/>
				</xsl:apply-templates>
			</xsl:if>
        ...
        <xsl:if test="not(ancestor::wp:anchor/@relativeHeight)">
          ...
          <xsl:choose>
            <xsl:when test ="contains(ancestor::wp:anchor/wp:docPr/@name,'组合') or contains(ancestor::wp:inline/wp:docPr/@name,'组合')">
              <xsl:value-of select="concat('grpspObj',./wpg:cNvPr/@id)"/>
            </xsl:when>
          ...
          <!--end-->
			...
			<图:预定义图形_8018>
				...
				<图:属性_801D>
					<图:大小_8060>
						<xsl:attribute name="宽_C605">
							<xsl:value-of select="./wpg:grpSpPr/a:xfrm/a:ext/@cx div 12700"/>
						</xsl:attribute>
						<xsl:attribute name="长_C604">
							<xsl:value-of select="./wpg:grpSpPr/a:xfrm/a:ext/@cy div 12700"/>
						</xsl:attribute>
					</图:大小_8060>
					<xsl:choose>
						<xsl:when test="./wpg:grpSpPr/a:xfrm/@rot">
							<图:旋转角度_804D>
								<xsl:value-of select="./wpg:grpSpPr/a:xfrm/@rot div 60000"/>
							</图:旋转角度_804D>
						</xsl:when>
						<xsl:otherwise>
							<图:旋转角度_804D>0.0</图:旋转角度_804D>
					...
					<图:缩放是否锁定纵横比_8055><!--cxl,2012.3.6修改BUG-->
						<xsl:if test="./wpg:cNvGrpSpPr/a:grpSpLocks/@noChangeAspect='1'">
							<xsl:value-of select=" 'true'"/>
						</xsl:if>
						<xsl:if test="./wpg:cNvGrpSpPr/a:grpSpLocks/@noChangeAspect='0' or not(./wpg:cNvGrpSpPr/a:grpSpLocks/@noChangeAspect)">
							<xsl:value-of select="'false'"/>
						</xsl:if>
					</图:缩放是否锁定纵横比_8055>
					<图:是否打印对象_804E>true</图:是否打印对象_804E>					
      ...
      <xsl:if test ="$picFrom ='grpsp'">
        <xsl:if test ="./wpg:grpSpPr/a:xfrm/a:off/@x != '0' and ./wpg:grpSpPr/a:xfrm/a:off/@y != '0'">
          <图:组合位置_803B>
            <xsl:attribute name="x_C606">
              <xsl:value-of select="./wpg:grpSpPr/a:xfrm/a:off/@x div 12700"/>
            </xsl:attribute>
            <xsl:attribute name="y_C607">
              <xsl:value-of select="./wpg:grpSpPr/a:xfrm/a:off/@y div 12700"/>
            </xsl:attribute>
          </图:组合位置_803B>
        </xsl:if>
      </xsl:if>
  ...
  <!--@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@-->
	<xsl:template match="wps:wsp|pic:pic">
		<xsl:param name="picFrom"/>
		<xsl:variable name="number">
			<xsl:number format="1" level="any" count="v:rect | wp:anchor | wp:inline | v:shape"/>
		</xsl:variable>
    <xsl:variable name="style" select="@style"/>
      ...
      <xsl:attribute name="层次_8063"><!--cxl,2012.5.14图形叠放次序，暂时先这样处理，效果上没问题-->
        <xsl:if test="ancestor::wp:anchor/@relativeHeight">
          <xsl:value-of select="ancestor::wp:anchor/@relativeHeight"/>
        </xsl:if>
        <xsl:if test="not(ancestor::wp:anchor/@relativeHeight)">
        ...
        <xsl:attribute name="图:其他对象">
          <xsl:value-of select ="concat($picFrom,'OBJ',$number * 2)"/>
			...
			<图:预定义图形_8018>
				<图:类别_8019>
					<xsl:choose>
						<!--cxl2011/11/21增加了几种漏转的图形（包括七边形、乘号、六角星、七角星、10角星、16角星），与差异文档做了对应关系的修改（包括减号、L形）-->
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='rect'">11</xsl:when><!--矩形-->
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='parallelogram'">12</xsl:when><!--平行四边形-->
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='trapezoid'">13</xsl:when><!--梯形-->
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='diamond'">14</xsl:when><!--菱形-->
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='roundRect'">15</xsl:when><!--圆角矩形-->
            <xsl:when test="./wps:spPr/a:prstGeom/@prst='heptagon'">16</xsl:when><!--七边形转换为八边形-->
            <xsl:when test="./wps:spPr/a:prstGeom/@prst='octagon'">16</xsl:when><!--八边形-->
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='triangle'">17</xsl:when><!--三角形17代表等腰三角形-->
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='rtTriangle'">18</xsl:when><!--右三角形-->
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='ellipse'">19</xsl:when><!--椭圆-->
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='diagStripe'">18</xsl:when><!--斜纹-->
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='corner'">11</xsl:when><!--L形-->
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='halfFrame'">18</xsl:when><!--半闭框-->
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='decagon'">19</xsl:when><!--十边形转换为圆-->
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='dodecagon'">19</xsl:when><!--十二边形转换为圆-->
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='pie'">19</xsl:when><!--饼形-->
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='chord'">19</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='teardrop'">19</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='cloud'">19</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='rightArrow'">21</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='leftArrow'">22</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='upArrow'">23</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='downArrow'">24</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='leftRightArrow'">25</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='upDownArrow'">26</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='quadArrow'">27</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='leftRightUpArrow'">28</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='bentArrow'">29</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartProcess'">31</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartAlternateProcess'">32</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartDecision'">33</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartInputOutput'">34</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartPredefinedProcess'">35</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartInternalStorage'">36</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartDocument'">37</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartMultidocument'">38</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartTerminator'">39</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='star4'">43</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='star5'">44</xsl:when>
            <xsl:when test="./wps:spPr/a:prstGeom/@prst='star6'">45</xsl:when>
            <xsl:when test="./wps:spPr/a:prstGeom/@prst='star7'">45</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='star8'">45</xsl:when>
            <xsl:when test="./wps:spPr/a:prstGeom/@prst='star10'">46</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='star12'">46</xsl:when>
            <xsl:when test="./wps:spPr/a:prstGeom/@prst='star16'">46</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='star24'">47</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='star32'">48</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='ribbon'">410</xsl:when>
            
            <!--2014-02-24，wudi，云形标注，start-->
            <xsl:when test="./wps:spPr/a:prstGeom/@prst='cloudCallout'">54</xsl:when>
            <!--end-->
            
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='line'">61</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='straightConnector1'">71</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='hexagon'">110</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='pentagon'">112</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='can'">113</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='cube'">114</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='bevel'">115</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='foldedCorner'">116</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='smileyFace'">117</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='donut'">118</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='blockArc'">120</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='heart'">121</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='lightningBolt'">122</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='sun'">123</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='moon'">124</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='arc'">125</xsl:when><!--弧形-->
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='bracketPair'">126</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='bracePair'">127</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='plaque'">128</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='leftBracket'">129</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='rightBracket'">130</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='leftBrace'">131</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='rightBrace'">132</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='uturnArrow'">210</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='leftUpArrow'">211</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='bentUpArrow'">212</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='curvedRightArrow'">213</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='curvedLeftArrow'">214</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='curvedUpArrow'">215</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='curvedDownArrow'">216</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='stripedRightArrow'">217</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='notchedRightArrow'">218</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='chevron'">220</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='rightArrowCallout'">221</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='leftArrowCallout'">222</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='upArrowCallout'">223</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='downArrowCallout'">224</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='leftRightArrowCallout'">225</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='upDownArrowCallout'">226</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='circularArrow'">228</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartPreparation'">310</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartManualInput'">311</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartManualOperation'">312</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartConnector'">313</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartOffpageConnector'">314</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartPunchedCard'">315</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartPunchedTape'">316</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartSummingJunction'">317</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartOr'">318</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartCollate'">319</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartSort'">320</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartExtract'">321</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartMerge'">322</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartDelay'">324</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartMagneticDisk'">326</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartDisplay'">328</xsl:when>
						<!--<xsl:when test="a:prstGeom/@prst='ribbon2'">410</xsl:when>-->
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='ribbon2'">49</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='ellipseRibbon'">412</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='ellipseRibbon2'">411</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='verticalScroll'">413</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='horizontalScroll'">414</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='wave'">415</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='doubleWave'">416</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='noSmoking'">119</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='plus'">111</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='mathPlus'">111</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='mathMinus'">11</xsl:when><!--减号转成矩形-->
            <xsl:when test="./wps:spPr/a:prstGeom/@prst='mathMultiply'">111</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='mathDivide'">111</xsl:when>
						<!--xsl:when test="a:prstGeom/@prst='straightConnector1'">71</xsl:when-->
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='irregularSeal1'">41</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='irregularSeal2'">42</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='wedgeRectCallout'">51</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='wedgeRoundRectCallout'">52</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='wedgeEllipseCallout'">53</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='borderCallout1'">56</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='borderCallout2'">57</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='borderCallout3'">58</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='accentCallout1'">510</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='accentCallout2'">511</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='accentCallout3'">512</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='callout1'">514</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='callout2'">515</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='callout3'">516</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='accentBorderCallout1'">518</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='accentBorderCallout2'">519</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='accentBorderCallout3'">520</xsl:when>
						<!--xsl:when test="a:prstGeom/@prst='straightConnector1'">62</xsl:when-->
						<!--<xsl:when test="./wps:spPr/a:prstGeom/@prst='curvedConnector3'">64</xsl:when>-->
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='bentConnector3'">74</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='curvedConnector3'">77</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='homePlate'">219</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='quadArrowCallout'">227</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartOnlineStorage'">323</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartMagneticTape'">325</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartMagneticDrum'">327</xsl:when>
						<xsl:otherwise>11</xsl:otherwise>
					</xsl:choose>
				</图:类别_8019>
				<图:名称_801A>
					<xsl:choose>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='rect'">Rectangle</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='parallelogram'">Parallelogram</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='trapezoid'">Trapezoid</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='diamond'">Diamond</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='roundRect'">Rounded Rectangle</xsl:when>
            <xsl:when test="./wps:spPr/a:prstGeom/@prst='heptagon'">Octagon</xsl:when><!--七边形转换为八边形-->
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='octagon'">Octagon</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='triangle'">Isosceles Triangle</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='rtTriangle'">Right Triangle</xsl:when>
						<!--斜纹，半闭框，L形转换为三角形-->
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='diagStripe'">Right Triangle</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='corner'">Rectangle</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='halfFrame'">Right Triangle</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='ellipse'">Oval</xsl:when>
						<!--2011：十边形，十二边形，饼，泪滴，弦月，云，转换为圆-->
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='decagon'">Oval</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='dodecagon'">Oval</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='pie'">Oval</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='chord'">Oval</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='teardrop'">Oval</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='cloud'">Oval</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='rightArrow'">Right Arrow</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='leftArrow'">Left Arrow</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='upArrow'">Up Arrow</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='downArrow'">Down Arrow</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='leftRightArrow'">Left-Right Arrow</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='upDownArrow'">Up-Down Arrow</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='quadArrow'">Quad Arrow</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='leftRightUpArrow'">Left-Right-Up Arrow</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='bentArrow'">Bent Arrow</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartProcess'">Process</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartAlternateProcess'">Alternate Process</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartDecision'">Decision</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartProcess'">Process</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartPredefinedProcess'">Predefined Process</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartInputOutput'">Data</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartInternalStorage'">Internal Storage</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartDocument'">Document</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartMultidocument'">Multidocument</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartTerminator'">Terminator</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='star4'">4-Point Star</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='star5'">5-Point Star</xsl:when>
						<!--2011：6、7角形转换为8角形-->
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='star6'">8-Point Star</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='star7'">8-Point Star</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='star8'">8-Point Star</xsl:when>
						<!--2011-：10、12角形转换为16角形-->
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='star10'">16-Point Star</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='star12'">16-Point Star</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='star16'">16-Point Star</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='star24'">24-Point Star</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='star32'">32-Point Star</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='ribbon'">Down Ribbon</xsl:when>
            
            <!--2014-02-24，wudi，云形标注，start-->
            <xsl:when test="./wps:spPr/a:prstGeom/@prst='cloudCallout'">Cloud Callout</xsl:when>
            <!--end-->
            
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='line'">Line</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='straightConnector1'">Straight Connector</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='hexagon'">Hexagon</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='pentagon'">Regual Pentagon</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='can'">Can</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='cube'">Cube</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='bevel'">Bevel</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='foldedCorner'">Folded Corner</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='smileyFace'">Smiley Face</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='donut'">Donut</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='blockArc'">Block Arc</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='heart'">Heart</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='lightningBolt'">Lightning</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='sun'">Sun</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='moon'">Moon</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='arc'">Arc</xsl:when><!--弧形-->
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='bracketPair'">Double Bracket</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='bracePair'">Double Brace</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='plaque'">Plaque</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='leftBracket'">Left Bracket</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='rightBracket'">Right Bracket</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='leftBrace'">Left Brace</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='rightBrace'">Right Brace</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='uturnArrow'">U-Turn Arrow</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='leftUpArrow'">Left-Up Arrow</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='bentUpArrow'">Bent-Up Arrow</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='curvedRightArrow'">Curved Right Arrow</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='curvedLeftArrow'">Curved Left Arrow</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='curvedUpArrow'">Curved Up Arrow</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='curvedDownArrow'">Curved Down Arrow</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='stripedRightArrow'">Striped Right Arrow</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='notchedRightArrow'">Notched Right Arrow</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='chevron'">Chevron Arrow</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='rightArrowCallout'">Right Arrow Callout</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='leftArrowCallout'">Left Arrow Callout</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='upArrowCallout'">Up Arrow Callout</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='downArrowCallout'">Down Arrow Callout</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='leftRightArrowCallout'">Left-Right Arrow Callout</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='upDownArrowCallout'">Up-Down Arrow Callout</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='circularArrow'">Circular Arrow</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartPreparation'">Preparation</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartManualInput'">Manual Input</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartManualOperation'">Manual Operation</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartConnector'">Connector</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartOffpageConnector'">Off-page Connector</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartPunchedCard'">Card</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartPunchedTape'">Punched Tape</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartSummingJunction'">Summing Junction</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartOr'">Or</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartCollate'">Collate</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartSort'">Sort</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartExtract'">Extract</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartMerge'">Merge</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartDelay'">Delay</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartMagneticDisk'">Magnetic Disk</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartDisplay'">Display</xsl:when>
						<!--zhaobj-->
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='ribbon2'">Up Ribbon</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='ellipseRibbon'">Curved Down Ribbon</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='ellipseRibbon2'">Curved Up Ribbon</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='verticalScroll'">Vertical Scroll</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='horizontalScroll'">Horizontal Scroll</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='wave'">Wave</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='doubleWave'">Double Wave</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='noSmoking'">No Symbol</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='plus'">Cross</xsl:when>
						<!--2011：加号、减号、除号转换为十字-->
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='mathPlus'">Cross</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='mathMinus'">Rectangle</xsl:when>
            <xsl:when test="./wps:spPr/a:prstGeom/@prst='mathMultiply'">Cross</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='mathDivide'">Cross</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='irregularSeal1'">Explosion 1</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='irregularSeal2'">Explosion 2</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='wedgeRectCallout'">Rectangular Callout</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='wedgeRoundRectCallout'">Rounded Rectangular Callout</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='wedgeEllipseCallout'">Oval Callout</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='borderCallout1'">Line Callout2</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='borderCallout2'">Line Callout3</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='borderCallout3'">Line Callout4</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='accentCallout1'">Line Callout2(Accent Bar)</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='accentCallout2'">Line Callout3(Accent Bar)</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='accentCallout3'">Line Callout4(Accent Bar)</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='callout1'">Line Callout2(No Border)</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='callout2'">Line Callout3(No Border)</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='callout3'">Line Callout4(No Border)</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='accentBorderCallout1'">Line Callout2(Border and Accent Bar)</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='accentBorderCallout2'">Line Callout3(Border and Accent Bar)</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='accentBorderCallout3'">Line Callout4(Border and Accent Bar)</xsl:when>
						<!--xsl:when test="a:prstGeom/@prst='straightConnector1'">Arrow</xsl:when-->
						<!--xsl:when test="a:prstGeom/@prst='curvedConnector3'">Curve</xsl:when-->
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='bentConnector3'">Elbow Connector</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='curvedConnector3'">Curved Connector</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='curvedConnector4'">Curved Connector</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='homePlate'">Pentagon Arrow</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='quadArrowCallout'">Quad Arrow Callout</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartOnlineStorage'">Stored Data</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartMagneticTape'">Sequential Access Storage</xsl:when>
						<xsl:when test="./wps:spPr/a:prstGeom/@prst='flowChartMagneticDrum'">Direct Access Storage</xsl:when>
						<!---->
						<xsl:when test="./wps:spPr/a:custGeom/*">Curve</xsl:when>
						<xsl:otherwise>Rectangle</xsl:otherwise>
					</xsl:choose>
				</图:名称_801A>
				<图:生成软件_801B>UOFTranslator</图:生成软件_801B>
				<图:属性_801D>
					<xsl:choose>
						<xsl:when test="./wps:spPr/a:gradFill"><!--渐变填充-->
							<图:填充_804C>
								<xsl:apply-templates select="wps:spPr/a:gradFill"/>
							</图:填充_804C>
						</xsl:when>
						<xsl:when test="./wps:spPr/a:pattFill"><!--图案填充-->
							<图:填充_804C>
								<xsl:apply-templates select="wps:spPr/a:pattFill"/>
							</图:填充_804C>
						</xsl:when>
						<xsl:when test="./wps:spPr/a:solidFill"><!--纯色填充-->
							<图:填充_804C>
								<图:颜色_8004>
									<xsl:apply-templates select="wps:spPr/a:solidFill"/>
								</图:颜色_8004>
							</图:填充_804C>
						</xsl:when>
						<xsl:when test="./wps:spPr/a:blipFill"><!--图片填充-->
							<图:填充_804C>
								<xsl:apply-templates select="wps:spPr/a:blipFill">
									<xsl:with-param name="picturefrom" select="$picFrom"/>
								</xsl:apply-templates>
							</图:填充_804C>
						</xsl:when>
						<xsl:when test="./wps:spPr/a:grpFill"><!--组合图形填充-->
							<xsl:apply-templates select="./wps:spPr/a:grpFill"/>
						</xsl:when>
						<xsl:when test="./wps:spPr/a:noFill"/>
            ...
            <!--有style的情况-->
						<!-- <xsl:when test="following-sibling::p:style/a:fillRef">-->
						<xsl:when test="./wps:style/a:fillRef[@idx!='0' and @idx!='1000']">
							<图:填充_804C>
								<图:颜色_8004>
									<xsl:apply-templates select="wps:style/a:fillRef"/>
								</图:颜色_8004>
              ...
              <图:填充_804C>
                <图:颜色_8004>auto</图:颜色_8004>
					...
					<图:大小_8060>
						<xsl:attribute name="宽_C605">
							<xsl:if test="./wps:spPr/a:xfrm/a:ext/@cx">
								<xsl:value-of select="./wps:spPr/a:xfrm/a:ext/@cx div 12700"/>
							</xsl:if>
							<xsl:if test="./pic:spPr/a:xfrm/a:ext/@cx">
								<xsl:value-of select="./pic:spPr/a:xfrm/a:ext/@cx div 12700"/>
							</xsl:if>
						...
						<xsl:attribute name="长_C604">
							<xsl:if test="./wps:spPr/a:xfrm/a:ext/@cy">
								<xsl:value-of select="./wps:spPr/a:xfrm/a:ext/@cy div 12700"/>
							</xsl:if>
							<xsl:if test="./pic:spPr/a:xfrm/a:ext/@cy">
								<xsl:value-of select="./pic:spPr/a:xfrm/a:ext/@cy div 12700"/>
							</xsl:if>
					...
					<图:线_8057>
						<xsl:choose>
							<xsl:when test="./wps:spPr/a:ln">
								<xsl:apply-templates select="wps:spPr/a:ln" mode="linecolor"/>
							</xsl:when>
							<xsl:when test="./wps:style/a:lnRef/*">
								<xsl:for-each select="./wps:style/a:lnRef">
									<图:线颜色_8058>
										<xsl:call-template name="lnRefcolor"/>
									</图:线颜色_8058>
              ...
              <!--<xsl:when test ="not(./pic:spPr/a:ln) and not(./wps:spPr/a:ln) and not(./wps:style/a:lnRef/*)">
                <图:线颜色_8058>#000000</图:线颜色_8058>
              </xsl:when>-->
              <!--end-->
						...
						<图:线类型_8059>
							<xsl:attribute name="线型_805A">
								<xsl:choose>
									<xsl:when test="./wps:spPr/a:ln/@cmpd='sng'">single</xsl:when>
									<xsl:when test="./wps:spPr/a:ln/@cmpd='dbl'">double</xsl:when>
									<xsl:when test="./wps:spPr/a:ln/@cmpd='thickThin'">thin-thick</xsl:when>
									<xsl:when test="./wps:spPr/a:ln/@cmpd='thinThick'">thick-thin</xsl:when>
									<xsl:when test="./wps:spPr/a:ln/@cmpd='tri'">thick-between-thin</xsl:when>
									<xsl:otherwise>single</xsl:otherwise>
								</xsl:choose>
							</xsl:attribute>
							<xsl:choose>
								<xsl:when test="./wps:spPr/a:ln/a:prstDash/@val">
									<xsl:attribute name="虚实_805B">
										<xsl:choose>
											<xsl:when test="./wps:spPr/a:ln/a:prstDash/@val='solid'">
												<xsl:value-of select="'solid'"/>
											</xsl:when>
											<!--圆点线-->
											<xsl:when test="./wps:spPr/a:ln/a:prstDash/@val='sysDot'">
												<xsl:value-of select="'round-dot'"/>
											</xsl:when>
											<!--方点线-->
											<xsl:when test="./wps:spPr/a:ln/a:prstDash/@val='sysDash'">
												<xsl:value-of select="'square-dot'"/>
											</xsl:when>
											<!--短划线->虚线-->
											<xsl:when test="./wps:spPr/a:ln/a:prstDash/@val='dash'">
												<xsl:value-of select="'dash'"/>
											</xsl:when>
											<!--划线-点->点虚线-->
											<xsl:when test="./wps:spPr/a:ln/a:prstDash/@val='dashDot'">
												<xsl:value-of select="'dash-dot'"/>
											</xsl:when>
											<!--长划线->长虚线-->
											<xsl:when test="./wps:spPr/a:ln/a:prstDash/@val='lgDash'">
												<xsl:value-of select="'long-dash'"/>
											</xsl:when>
											<!--长划线-点-->
											<xsl:when test="./wps:spPr/a:ln/a:prstDash/@val='lgDashDot'">
												<xsl:value-of select="'long-dash-dot'"/>
											</xsl:when>
											<!--长划线-点-点-->
											<xsl:when test="./wps:spPr/a:ln/a:prstDash/@val='lgDashDotDot'">
												<xsl:value-of select="'dash-dot-dot'"/>
											</xsl:when>
											<!--其它情况处理>
            ...
            <xsl:if test="name(.)='wps:wsp'">
						  <xsl:choose>             
							  <xsl:when test="./wps:spPr/a:ln/@w">
								  <图:线粗细_805C>
									  <xsl:value-of select="./wps:spPr/a:ln/@w div 12700"/>
								  </图:线粗细_805C>
							  </xsl:when>
							  <xsl:otherwise>
                  ...
                  <图:线粗细_805C>
                  <xsl:choose>
                    <xsl:when test="./wps:spPr/a:prstGeom/@prst='line'">0.75</xsl:when><!--直接连接符 2-->
                    <xsl:when test="./wps:spPr/a:prstGeom/@prst='straightConnector1'">0.75</xsl:when><!--直接箭头连接符 3，直接箭头连接符 4-->
                    <xsl:when test="./wps:spPr/a:prstGeom/@prst='arc'">0.75</xsl:when><!--弧形-->
                    <xsl:when test="./wps:spPr/a:prstGeom/@prst='bracketPair'">0.75</xsl:when><!--双括号 61-->
                    <xsl:when test="./wps:spPr/a:prstGeom/@prst='bracePair'">0.75</xsl:when><!--双大括号 62-->
                    <xsl:when test="./wps:spPr/a:prstGeom/@prst='leftBracket'">0.75</xsl:when><!--左中括号 66-->
                    <xsl:when test="./wps:spPr/a:prstGeom/@prst='rightBracket'">0.75</xsl:when><!--右中括号 63-->
                    <xsl:when test="./wps:spPr/a:prstGeom/@prst='leftBrace'">0.75</xsl:when><!--左大括号 64-->
                    <xsl:when test="./wps:spPr/a:prstGeom/@prst='rightBrace'">0.75</xsl:when><!--右大括号 65-->
                    <xsl:when test="./wps:spPr/a:prstGeom/@prst='bentConnector3'">0.75</xsl:when><!--肘形连接符 5，肘形连接符6，肘形连接符 7-->
                    <xsl:when test="./wps:spPr/a:prstGeom/@prst='curvedConnector3'">0.75</xsl:when><!--曲线连接符8，曲线连接符9，曲线连接符11-->
                    <xsl:otherwise>2.0</xsl:otherwise>
                  </xsl:choose>
                  </图:线粗细_805C>
                  <!--end-->
            ...
            <xsl:if test="name(.)='pic:pic'">
              <xsl:choose>
                <xsl:when test="./pic:spPr/a:ln/@w"><!--cxl2011/12/9添加图片边框粗细-->
                  <图:线粗细_805C>
                    <xsl:value-of select="./pic:spPr/a:ln/@w div 12700"/>
                  </图:线粗细_805C>
                </xsl:when>
                <xsl:otherwise>
                  <图:线粗细_805C>0.0</图:线粗细_805C><!--原来是0.75，但发现WORD里预设为0.0磅-->
                </xsl:otherwise>
              </xsl:choose>
            </xsl:if>
					</图:线_8057>
					<xsl:choose>
						<xsl:when test="./wps:spPr/a:xfrm/@rot">
							<图:旋转角度_804D>
								<!-- 旋转角度<xsl:value-of select="(21600000-a:xfrm/@rot) div 60000"/>-->
								<xsl:value-of select="./wps:spPr/a:xfrm/@rot div 60000"/>
							</图:旋转角度_804D>
						</xsl:when>
						<xsl:otherwise>
							<图:旋转角度_804D>0.0</图:旋转角度_804D>
					...
					<!--zhaobj 箭头-->
					<xsl:if test="./wps:spPr/a:ln">          
						<xsl:apply-templates select="wps:spPr/a:ln" mode="Arrow"/>
					</xsl:if>
					<!--<xsl:if test="wp:cNvGraphicFramePr/a:graphicFrameLocks/@noChangeAspect='1'">
            <图:锁定纵横比 uof:locID="g0028">0</图:锁定纵横比>
          </xsl:if>-->
					...
					<图:缩放是否锁定纵横比_8055><!--cxl,2012.3.6修改BUG-->
						...
						<xsl:if test="$picFrom != 'grpsp'">
							<xsl:choose>
								<xsl:when test="./wps:cNvSpPr/a:spLocks/@noChangeAspect='1'"><!--/../@locked-->
									<xsl:value-of select="'true'"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="'false'"/>
					...
					<!--<图:Web文字 uof:locID="g0033"/>-->
					<!--zhaobj 透明度a:solidFill 还有 a:gradFill a:pattFill a:blipFill的情况-->
					<xsl:if test="./wps:spPr/a:ln/a:solidFill//a:alpha">
						<图:透明度_8050>
							<xsl:value-of select="round(100-(./wps:spPr/a:ln/a:solidFill//a:alpha/@val div 1000))"/>
						</图:透明度_8050>
					</xsl:if>
          
          <!--2012-01-25，wudi，OOX到UOF方向修复BUG：纯色填充的颜色透明度效果丢失，start-->
          <xsl:if test="./wps:spPr/a:solidFill//a:alpha">
            <图:透明度_8050>
              <xsl:value-of select="round(100-(./wps:spPr/a:solidFill//a:alpha/@val div 1000))"/>
            </图:透明度_8050>
          </xsl:if>
          <!--end-->
          
					<xsl:if test="./wps:spPr/a:effectLst/a:outerShdw"><!--外部阴影-->
						<xsl:apply-templates select="wps:spPr/a:effectLst/a:outerShdw" mode="shadow"/>
					</xsl:if>
					<xsl:if test="./wps:spPr/a:effectLst/a:innerShdw"><!--内部阴影-->
						<xsl:apply-templates select="wps:spPr/a:effectLst/a:innerShdw" mode="shadow"/>
					</xsl:if>
          ...
          <!--cxl2011/11/20开始增加三维效果转化代码*****************************************************************************-->
          <xsl:if test="./wps:spPr/a:scene3d">
            <图:三维效果_8061>
              <xsl:apply-templates select="./wps:spPr/a:scene3d"/>
            </图:三维效果_8061>
          </xsl:if>
				</图:属性_801D>
			</图:预定义图形_8018>
      <!--cxl2011/11/19添加图片数据引用,其它数据引用按范围文档不转************************************************-->
      <xsl:if test="./pic:blipFill"><!--./wps:spPr/a:blipFill|--> 
        <图:图片数据引用_8037>
          <xsl:variable name="picId">
            <xsl:if test="./pic:blipFill/a:blip/@r:embed">
              <xsl:value-of select="./pic:blipFill/a:blip/@r:embed"/>
            </xsl:if>
            <!--<xsl:if test="./wps:spPr/a:blipFill/a:blip/@r:embed">--><!--待做案例验证--><!--
              <xsl:value-of select="./wps:spPr/a:blipFill/a:blip/@r:embed"/>
            </xsl:if>-->
          </xsl:variable>
          <xsl:variable name="picName">
            <xsl:choose>
              ...
              <!--暂时先这样-->
				  ...
				  <xsl:variable name="pictype">
					...
					<xsl:if test="$picFrom != 'grpsp'">
						<xsl:value-of select="concat($picFrom,'Obj',$number * 2)"/><!--substring-after($picName,'/')-->
					</xsl:if>  
        </图:图片数据引用_8037>      
      </xsl:if>
      
      <!--2014-02-17，wudi，预定义图形-控制点的转换，start-->
      <xsl:choose>
        <!--椭圆形标注，圆角矩形标注，矩形标注，云形标注-->
        <xsl:when test ="(./wps:spPr/a:prstGeom/@prst='wedgeEllipseCallout' or ./wps:spPr/a:prstGeom/@prst='wedgeRoundRectCallout' or ./wps:spPr/a:prstGeom/@prst='wedgeRectCallout' or ./wps:spPr/a:prstGeom/@prst='cloudCallout') and ./*/a:prstGeom/a:avLst/a:gd">
        ...
        <xsl:when test ="(./wps:spPr/a:prstGeom/@prst='borderCallout1' or ./wps:spPr/a:prstGeom/@prst='accentCallout1' or ./wps:spPr/a:prstGeom/@prst='callout1' or ./wps:spPr/a:prstGeom/@prst='accentBorderCallout1') and ./*/a:prstGeom/a:avLst/a:gd">
          <xsl:variable name="adj1">
            <xsl:value-of select="./*/a:prstGeom/a:avLst/a:gd[@name = 'adj1']/@fmla"/>
          </xsl:variable>
          <xsl:variable name="adj2">
        ...
        <xsl:when test="(./wps:spPr/a:prstGeom/@prst='borderCallout2' or ./wps:spPr/a:prstGeom/@prst='accentCallout2' or ./wps:spPr/a:prstGeom/@prst='callout2' or ./wps:spPr/a:prstGeom/@prst='accentBorderCallout2') and ./*/a:prstGeom/a:avLst/a:gd">
          <xsl:variable name="adj1">
            <xsl:value-of select="./*/a:prstGeom/a:avLst/a:gd[@name = 'adj1']/@fmla"/>
          </xsl:variable>
          <xsl:variable name="adj2">
        ...
        <xsl:when test="(./wps:spPr/a:prstGeom/@prst='borderCallout3' or ./wps:spPr/a:prstGeom/@prst='accentCallout3' or ./wps:spPr/a:prstGeom/@prst='callout3' or ./wps:spPr/a:prstGeom/@prst='accentBorderCallout3') and ./*/a:prstGeom/a:avLst/a:gd">
          <xsl:variable name="adj1">
            <xsl:value-of select="./*/a:prstGeom/a:avLst/a:gd[@name = 'adj1']/@fmla"/>
          </xsl:variable>
          <xsl:variable name="adj2">
        ...
        <xsl:when test ="(./wps:spPr/a:prstGeom/@prst='star4' or ./wps:spPr/a:prstGeom/@prst='star8' or ./wps:spPr/a:prstGeom/@prst='star16' or ./wps:spPr/a:prstGeom/@prst='star24' or ./wps:spPr/a:prstGeom/@prst='star32' or ./wps:spPr/a:prstGeom/@prst='star6' or ./wps:spPr/a:prstGeom/@prst='star7' or ./wps:spPr/a:prstGeom/@prst='star10' or ./wps:spPr/a:prstGeom/@prst='star12') and ./*/a:prstGeom/a:avLst/a:gd">
        ...
        <!--上凸带形-->
        <xsl:when test ="(./wps:spPr/a:prstGeom/@prst='ribbon2') and ./*/a:prstGeom/a:avLst/a:gd">
          <xsl:variable name="adj1">
            <xsl:value-of select="./*/a:prstGeom/a:avLst/a:gd[@name = 'adj1']/@fmla"/>
          </xsl:variable>
          <xsl:variable name="adj2">
        ...
        <!--前凸带形-->
        <xsl:when test ="(./wps:spPr/a:prstGeom/@prst='ribbon') and ./*/a:prstGeom/a:avLst/a:gd">
          <xsl:variable name="adj1">
            <xsl:value-of select="./*/a:prstGeom/a:avLst/a:gd[@name = 'adj1']/@fmla"/>
          </xsl:variable>
          <xsl:variable name="adj2">
        ...
        <!--上凸弯带形-->
        <xsl:when test ="(./wps:spPr/a:prstGeom/@prst='ellipseRibbon2') and ./*/a:prstGeom/a:avLst/a:gd">
          <xsl:variable name="adj1">
            <xsl:value-of select="./*/a:prstGeom/a:avLst/a:gd[@name = 'adj1']/@fmla"/>
          </xsl:variable>
          <xsl:variable name="adj2">
        ...
        <!--前凸弯带形-->
        <xsl:when test ="(./wps:spPr/a:prstGeom/@prst='ellipseRibbon') and ./*/a:prstGeom/a:avLst/a:gd">
          <xsl:variable name="adj1">
            <xsl:value-of select="./*/a:prstGeom/a:avLst/a:gd[@name = 'adj1']/@fmla"/>
          </xsl:variable>
          <xsl:variable name="adj2">
        ...
        <!--竖卷形，横卷形-->
        <xsl:when test ="(./wps:spPr/a:prstGeom/@prst='verticalScroll' or ./wps:spPr/a:prstGeom/@prst='horizontalScroll') and ./*/a:prstGeom/a:avLst/a:gd">
          <xsl:variable name="adj">
            <xsl:value-of select="./*/a:prstGeom/a:avLst/a:gd[@name = 'adj']/@fmla"/>
          </xsl:variable>
          <xsl:variable name="tmp">
        ...
        <!--波形，双波形-->
        <xsl:when test ="(./wps:spPr/a:prstGeom/@prst='wave' or ./wps:spPr/a:prstGeom/@prst='doubleWave') and ./*/a:prstGeom/a:avLst/a:gd">
          <xsl:variable name="adj1">
            <xsl:value-of select="./*/a:prstGeom/a:avLst/a:gd[@name = 'adj1']/@fmla"/>
          </xsl:variable>
          <xsl:variable name="adj2">
        ...
        <!--右箭头，虚尾箭头，燕尾形箭头-->
        <xsl:when test ="(./wps:spPr/a:prstGeom/@prst='rightArrow' or ./wps:spPr/a:prstGeom/@prst='stripedRightArrow' or ./wps:spPr/a:prstGeom/@prst='notchedRightArrow') and ./*/a:prstGeom/a:avLst/a:gd">
          <xsl:variable name="adj1">
            <xsl:value-of select="./*/a:prstGeom/a:avLst/a:gd[@name = 'adj1']/@fmla"/>
          </xsl:variable>
          <xsl:variable name="adj2">
          ...
          <xsl:variable name="width">
            <xsl:value-of select="./wps:spPr/a:xfrm/a:ext/@cx"/>
          </xsl:variable>
          <xsl:variable name="height">
            <xsl:value-of select="./wps:spPr/a:xfrm/a:ext/@cy"/>
          </xsl:variable>
          <图:控制点_8039>
            <xsl:attribute name="x_C606">
        ...
        <!--左箭头，左右箭头-->
        <xsl:when test ="(./wps:spPr/a:prstGeom/@prst='leftArrow' or ./wps:spPr/a:prstGeom/@prst='leftRightArrow') and ./*/a:prstGeom/a:avLst/a:gd">
          <xsl:variable name="adj1">
            <xsl:value-of select="./*/a:prstGeom/a:avLst/a:gd[@name = 'adj1']/@fmla"/>
          </xsl:variable>
          <xsl:variable name="adj2">
        ...
        <!--上箭头-->
        <xsl:when test ="(./wps:spPr/a:prstGeom/@prst='upArrow') and ./*/a:prstGeom/a:avLst/a:gd">
          <xsl:variable name="adj1">
            <xsl:value-of select="./*/a:prstGeom/a:avLst/a:gd[@name = 'adj1']/@fmla"/>
          </xsl:variable>
          <xsl:variable name="adj2">
        ...
        <!--上下箭头-->
        <xsl:when test ="(./wps:spPr/a:prstGeom/@prst='upDownArrow') and ./*/a:prstGeom/a:avLst/a:gd">
          <xsl:variable name="adj1">
            <xsl:value-of select="./*/a:prstGeom/a:avLst/a:gd[@name = 'adj1']/@fmla"/>
          </xsl:variable>
          <xsl:variable name="adj2">
        ...
        <!--下箭头-->
        <xsl:when test ="(./wps:spPr/a:prstGeom/@prst='downArrow') and ./*/a:prstGeom/a:avLst/a:gd">
          <xsl:variable name="adj1">
            <xsl:value-of select="./*/a:prstGeom/a:avLst/a:gd[@name = 'adj1']/@fmla"/>
          </xsl:variable>
          <xsl:variable name="adj2">
        ...
        <!--十字箭头，丁字箭头-->
        <xsl:when test ="(./wps:spPr/a:prstGeom/@prst='quadArrow' or ./wps:spPr/a:prstGeom/@prst='leftRightUpArrow') and ./*/a:prstGeom/a:avLst/a:gd">
          <xsl:variable name="adj1">
            <xsl:value-of select="./*/a:prstGeom/a:avLst/a:gd[@name = 'adj1']/@fmla"/>
          </xsl:variable>
          <xsl:variable name="adj2">
        ...
        <!--圆角右箭头-->
        <xsl:when test ="(./wps:spPr/a:prstGeom/@prst='bentArrow') and ./*/a:prstGeom/a:avLst/a:gd">
          <xsl:variable name="adj1">
            <xsl:value-of select="./*/a:prstGeom/a:avLst/a:gd[@name = 'adj1']/@fmla"/>
          </xsl:variable>
          <xsl:variable name="adj3">
        ...
        <!--直角双向箭头，直角上箭头-->
        <xsl:when test ="(./wps:spPr/a:prstGeom/@prst='leftUpArrow' or ./wps:spPr/a:prstGeom/@prst='bentUpArrow') and ./*/a:prstGeom/a:avLst/a:gd">
          <xsl:variable name="adj1">
            <xsl:value-of select="./*/a:prstGeom/a:avLst/a:gd[@name = 'adj1']/@fmla"/>
          </xsl:variable>
          <xsl:variable name="adj2">
        ...
        <!--左弧形箭头，显示效果有差异-->
        <xsl:when test ="(./wps:spPr/a:prstGeom/@prst='curvedRightArrow') and ./*/a:prstGeom/a:avLst/a:gd">
          <xsl:variable name="adj1">
            <xsl:value-of select="./*/a:prstGeom/a:avLst/a:gd[@name = 'adj1']/@fmla"/>
          </xsl:variable>
          <xsl:variable name="adj2">
        ...
        <!--右弧形箭头，显示效果有差异-->
        <xsl:when test ="(./wps:spPr/a:prstGeom/@prst='curvedLeftArrow') and ./*/a:prstGeom/a:avLst/a:gd">
          <xsl:variable name="adj1">
            <xsl:value-of select="./*/a:prstGeom/a:avLst/a:gd[@name = 'adj1']/@fmla"/>
          </xsl:variable>
          <xsl:variable name="adj2">
        ...
        <!--下弧形箭头，显示效果有差异-->
        <xsl:when test ="(./wps:spPr/a:prstGeom/@prst='curvedUpArrow') and ./*/a:prstGeom/a:avLst/a:gd">
          <xsl:variable name="adj1">
            <xsl:value-of select="./*/a:prstGeom/a:avLst/a:gd[@name = 'adj1']/@fmla"/>
          </xsl:variable>
          <xsl:variable name="adj2">
        ...
        <!--上弧形箭头，显示效果有差异-->
        <xsl:when test ="(./wps:spPr/a:prstGeom/@prst='curvedDownArrow') and ./*/a:prstGeom/a:avLst/a:gd">
          <xsl:variable name="adj1">
            <xsl:value-of select="./*/a:prstGeom/a:avLst/a:gd[@name = 'adj1']/@fmla"/>
          </xsl:variable>
          <xsl:variable name="adj2">
        ...
        <!--五边形，燕尾形-->
        <xsl:when test ="(./wps:spPr/a:prstGeom/@prst='homePlate' or ./wps:spPr/a:prstGeom/@prst='chevron') and ./*/a:prstGeom/a:avLst/a:gd">
          <xsl:variable name="adj">
            <xsl:value-of select="./*/a:prstGeom/a:avLst/a:gd[@name = 'adj']/@fmla"/>
          </xsl:variable>
          <xsl:variable name="tmp">
        ...
        <!--右箭头标注-->
        <xsl:when test="(./wps:spPr/a:prstGeom/@prst='rightArrowCallout') and ./*/a:prstGeom/a:avLst/a:gd">
          <xsl:variable name="adj1">
            <xsl:value-of select="./*/a:prstGeom/a:avLst/a:gd[@name = 'adj1']/@fmla"/>
          </xsl:variable>
          <xsl:variable name="adj2">
        ...
        <!--左箭头标注-->
        <xsl:when test="(./wps:spPr/a:prstGeom/@prst='leftArrowCallout') and ./*/a:prstGeom/a:avLst/a:gd">
          <xsl:variable name="adj1">
            <xsl:value-of select="./*/a:prstGeom/a:avLst/a:gd[@name = 'adj1']/@fmla"/>
          </xsl:variable>
          <xsl:variable name="adj2">
        ...
        <!--上箭头标注-->
        <xsl:when test="(./wps:spPr/a:prstGeom/@prst='upArrowCallout') and ./*/a:prstGeom/a:avLst/a:gd">
          <xsl:variable name="adj1">
            <xsl:value-of select="./*/a:prstGeom/a:avLst/a:gd[@name = 'adj1']/@fmla"/>
          </xsl:variable>
          <xsl:variable name="adj2">
        ...
        <!--下箭头标注-->
        <xsl:when test="(./wps:spPr/a:prstGeom/@prst='downArrowCallout') and ./*/a:prstGeom/a:avLst/a:gd">
          <xsl:variable name="adj1">
            <xsl:value-of select="./*/a:prstGeom/a:avLst/a:gd[@name = 'adj1']/@fmla"/>
          </xsl:variable>
          <xsl:variable name="adj2">
        ...
        <!--左右箭头标注-->
        <xsl:when test="(./wps:spPr/a:prstGeom/@prst='leftRightArrowCallout') and ./*/a:prstGeom/a:avLst/a:gd">
          <xsl:variable name="adj1">
            <xsl:value-of select="./*/a:prstGeom/a:avLst/a:gd[@name = 'adj1']/@fmla"/>
          </xsl:variable>
          <xsl:variable name="adj2">
        ...
        <!--十字箭头标注-->
        <xsl:when test="(./wps:spPr/a:prstGeom/@prst='quadArrowCallout') and ./*/a:prstGeom/a:avLst/a:gd">
          <xsl:variable name="adj1">
            <xsl:value-of select="./*/a:prstGeom/a:avLst/a:gd[@name = 'adj1']/@fmla"/>
          </xsl:variable>
          <xsl:variable name="adj2">
        ...
        <xsl:when test="(./wps:spPr/a:prstGeom/@prst='triangle' or ./wps:spPr/a:prstGeom/@prst='octagon' or ./wps:spPr/a:prstGeom/@prst='plus' or ./wps:spPr/a:prstGeom/@prst='plaque' or ./wps:spPr/a:prstGeom/@prst='cube' or ./wps:spPr/a:prstGeom/@prst='bevel' or ./wps:spPr/a:prstGeom/@prst='sun' or ./wps:spPr/a:prstGeom/@prst='moon' or ./wps:spPr/a:prstGeom/@prst='bracketPair' or ./wps:spPr/a:prstGeom/@prst='bracePair') and ./*/a:prstGeom/a:avLst/a:gd">
          <xsl:variable name="adj">
            <xsl:value-of select="./*/a:prstGeom/a:avLst/a:gd[@name = 'adj']/@fmla"/>
          </xsl:variable>
          <xsl:variable name="tmp">
        ...
        <xsl:when test="(./wps:spPr/a:prstGeom/@prst='parallelogram' or ./wps:spPr/a:prstGeom/@prst='trapezoid' or ./wps:spPr/a:prstGeom/@prst='hexagon' or ./wps:spPr/a:prstGeom/@prst='donut' or ./wps:spPr/a:prstGeom/@prst='noSmoking') and ./*/a:prstGeom/a:avLst/a:gd">
          <xsl:variable name="adj">
            <xsl:value-of select="./*/a:prstGeom/a:avLst/a:gd[@name = 'adj']/@fmla"/>
          </xsl:variable>
          <xsl:variable name="tmp">
        ...
        <!--圆柱形-->
        <xsl:when test="(./wps:spPr/a:prstGeom/@prst='can') and ./*/a:prstGeom/a:avLst/a:gd">
          <xsl:variable name="adj">
            <xsl:value-of select="./*/a:prstGeom/a:avLst/a:gd[@name = 'adj']/@fmla"/>
          </xsl:variable>
          <xsl:variable name="tmp">
        ...
        <!--折角形-->
        <xsl:when test="(./wps:spPr/a:prstGeom/@prst='foldedCorner') and ./*/a:prstGeom/a:avLst/a:gd">
          <xsl:variable name="adj">
            <xsl:value-of select="./*/a:prstGeom/a:avLst/a:gd[@name = 'adj']/@fmla"/>
          </xsl:variable>
          <xsl:variable name="tmp">
        ...
        <!--笑脸-->
        <xsl:when test="(./wps:spPr/a:prstGeom/@prst='smileyFace') and ./*/a:prstGeom/a:avLst/a:gd">
          <xsl:variable name="adj">
            <xsl:value-of select="./*/a:prstGeom/a:avLst/a:gd[@name = 'adj']/@fmla"/>
          </xsl:variable>
          <xsl:variable name="tmp">
        ...
        <!--左中括号，右中括号-->
        <xsl:when test="(./wps:spPr/a:prstGeom/@prst='leftBracket' or ./wps:spPr/a:prstGeom/@prst='rightBracket') and ./*/a:prstGeom/a:avLst/a:gd">
          <xsl:variable name="adj">
            <xsl:value-of select="./*/a:prstGeom/a:avLst/a:gd[@name = 'adj']/@fmla"/>
          </xsl:variable>
          <xsl:variable name="tmp">
        ...
        <!--左大括号，右大括号-->
        <xsl:when test="(./wps:spPr/a:prstGeom/@prst='leftBrace' or ./wps:spPr/a:prstGeom/@prst='rightBrace') and ./*/a:prstGeom/a:avLst/a:gd">
          <xsl:variable name="adj1">
            <xsl:value-of select="./*/a:prstGeom/a:avLst/a:gd[@name = 'adj1']/@fmla"/>
          </xsl:variable>
          <xsl:variable name="adj2">
        ...
        <xsl:when test="(./wps:spPr/a:prstGeom/@prst='bentConnector3' or ./wps:spPr/a:prstGeom/@prst='curvedConnector3') and ./*/a:prstGeom/a:avLst/a:gd">
          <xsl:variable name="adj1">
            <xsl:value-of select="./*/a:prstGeom/a:avLst/a:gd[@name = 'adj1']/@fmla"/>
          </xsl:variable>
          <xsl:variable name="tmp">
      ...
      <!--2014-02-17，wudi，注释掉此部分代码，永中2013无此问题
      <xsl:choose>
        <xsl:when test ="./wps:spPr/a:prstGeom/@prst='borderCallout3' and not(./*/a:xfrm/@flipH)">
          <图:翻转_803A>
            <xsl:value-of select="'x'"/>
          </图:翻转_803A>
        </xsl:when>
        <xsl:when test ="./wps:spPr/a:prstGeom/@prst='accentCallout3' and not(./*/a:xfrm/@flipH)">
          <图:翻转_803A>
            <xsl:value-of select="'x'"/>
          </图:翻转_803A>
        </xsl:when>
        <xsl:when test ="./wps:spPr/a:prstGeom/@prst='callout3' and not(./*/a:xfrm/@flipH)">
          <图:翻转_803A>
            <xsl:value-of select="'x'"/>
          </图:翻转_803A>
        </xsl:when>
        <xsl:when test ="./wps:spPr/a:prstGeom/@prst='accentBorderCallout3' and not(./*/a:xfrm/@flipH)">
          <图:翻转_803A>
            <xsl:value-of select="'x'"/>
          </图:翻转_803A>
        </xsl:when>
        <xsl:when test ="./wps:spPr/a:prstGeom/@prst='wedgeRectCallout' and not(./*/a:xfrm/@flipH)">
          <图:翻转_803A>
            <xsl:value-of select="'x'"/>
          </图:翻转_803A>
        </xsl:when>
			...
			<xsl:if test="./*/a:xfrm/@flipH and not(./wps:spPr/a:prstGeom/@prst='borderCallout3') and not(./wps:spPr/a:prstGeom/@prst='accentCallout3') and not(./wps:spPr/a:prstGeom/@prst='callout3') and not(./wps:spPr/a:prstGeom/@prst='accentBorderCallout3')">
				<图:翻转_803A>
					<xsl:value-of select="'x'"/>
				</图:翻转_803A>
			</xsl:if>
      ...
      <!--end-->
			...
			<xsl:for-each select="./wps:txbx">
				<xsl:apply-templates select="." mode="文本"/><!--预定义图形中文本的转化-->
			</xsl:for-each>
			<xsl:if test="$picFrom ='grpsp'">
				<图:组合位置_803B>
					<xsl:attribute name="x_C606">
						<xsl:value-of select="./wps:spPr/a:xfrm/a:off/@x div 12700"/>
					</xsl:attribute>
					<xsl:attribute name="y_C607">
						<xsl:value-of select="./wps:spPr/a:xfrm/a:off/@y div 12700"/>
					</xsl:attribute>
				</图:组合位置_803B>
			</xsl:if>
		</图:图形_8062>
	...
	<xsl:template match="wps:spPr/a:effectLst/a:outerShdw" mode="shadow">
        ...
        <!--2013-03-06，wudi，修改if判断条件，修复预定义图形阴影-透明度转换错误BUG，start-->
				<xsl:when test="a:srgbClr/a:alpha">
					<xsl:if test="a:srgbClr/a:alpha/@val &lt; 20001">
						<xsl:attribute name="透明度_C61F">100</xsl:attribute>
					</xsl:if>
					<xsl:if test="a:srgbClr/a:alpha/@val &gt; 20999">
						<xsl:attribute name="透明度_C61F">50</xsl:attribute>
					</xsl:if>
				</xsl:when>
				<xsl:when test="a:prstClr/a:alpha">
					<xsl:if test="a:prstClr/a:alpha/@val &lt; 20001">
						<xsl:attribute name="透明度_C61F">100</xsl:attribute>
					</xsl:if>
					<xsl:if test="a:prstClr/a:alpha/@val &gt; 20999">
						<xsl:attribute name="透明度_C61F">50</xsl:attribute>
					</xsl:if>
				</xsl:when>
				<xsl:when test="a:schemeClr/a:alpha">
					<xsl:if test="a:schemeClr/a:alpha/@val &lt; 20001">
						<xsl:attribute name="透明度_C61F">100</xsl:attribute>
					</xsl:if>
					<xsl:if test="a:schemeClr/a:alpha/@val &gt; 20999">
						<xsl:attribute name="透明度_C61F">50</xsl:attribute>
					</xsl:if>
				</xsl:when>
        <!--end-->
  ...
  <!--cxl2011/12/8修改-->
	...
	<xsl:template match="a:blipFill">
        ...
        <!--2014-03-06，wudi，增加心形 12和椭圆 9，start-->
				<xsl:if test="$picturefrom='grpsp' and not(ancestor::wps:wsp/wps:cNvPr/@name='五角星 17' or ancestor::wps:wsp/wps:cNvPr/@name='矩形 10' or ancestor::wps:wsp/wps:cNvPr/@name='心形 12' or ancestor::wps:wsp/wps:cNvPr/@name='椭圆 9')">
          <xsl:if test="../../wps:cNvPr/@id">
						<xsl:value-of select="concat('grpsppicObj',../../wps:cNvPr/@id)"/>
					</xsl:if>
				</xsl:if>
        <xsl:if test ="$picturefrom='grpsp' and (ancestor::wps:wsp/wps:cNvPr/@name='五角星 17' or ancestor::wps:wsp/wps:cNvPr/@name='矩形 10' or ancestor::wps:wsp/wps:cNvPr/@name='心形 12' or ancestor::wps:wsp/wps:cNvPr/@name='椭圆 9')">
  ...
  <!--预定义图形中文本的转换-->
    ...
    <图:文本_803C>
      ...
      <图:内容_8043>
        ...
        <xsl:for-each select=".//w:txbxContent/w:tbl">
        ...
        <xsl:for-each select=".//w:txbxContent/w:p | .//w:txbxContent/w:sdt/w:sdtContent/w:p">
          <xsl:call-template name="paragraph">
  ...
  <xsl:template match="wps:spPr/a:scene3d">
