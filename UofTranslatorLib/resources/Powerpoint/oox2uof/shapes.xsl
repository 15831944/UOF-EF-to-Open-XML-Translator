...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="p:spTree" mode="sp">
    ...
    <!--liuyangyang 2015-03-06 添加对公式的处理-->
    <xsl:for-each select="p:cxnSp|p:grpSp|p:pic|p:sp|p:graphicFrame|mc:AlternateContent">
      <xsl:if test="name(.) = 'mc:AlternateContent'">
        ...
        <图:预定义图形_8018>
          ...
          <图:图片数据引用_8037>
            <xsl:value-of select="concat(substring-before(../../../@id,'.xml'),./a:blip/@r:embed)"/>
          </图:图片数据引用_8037>
          <图:其他对象引用_8038>
            <xsl:value-of select="@id"/>
          </图:其他对象引用_8038>
    ...
    <!--guoyongbin 2014-03-06 修改图形批注引用-->
    <!--liuyangyang 15-03-11 修改transitional 版本批注丢失 start-->
    <!--<xsl:if test="//p:cmAuthorLst and ../../@commentID">-->
      <xsl:if test="//p:cmLst">
        <xsl:variable name="cmSlideID" select="../../@id"/>
        <xsl:for-each select="//p:cmLst">
          <xsl:if test="@refBy = $cmSlideID">
            <图:图形_8062>
              <xsl:attribute name="标识符_804B">
     ...
     <!--end liuyangyang 15-03-11 修改transitional 版本批注丢失-->
    </xsl:if>
    <!--enf -->
	  <!--添加对批注的图形引用 李娟-->
  ...
  <xsl:template match="p:graphicFrame">
    ...
    <xsl:if test="contains($graphicdataType,'ole')">
  ...
  <xsl:template  name="chart">
    ...
    <xsl:variable name="idofralationship">
      ...
      <图:图形_8062>
        ...
        <xsl:for-each select="//rel:Relationships[@id=$idofralationship]/rel:Relationship[@Type='http://schemas.openxmlformats.org/officeDocument/2006/relationships/chart']">
          <xsl:element name ="图:图片数据引用_8037">
    ...
    <!--2010-11-10罗文甜：增加OLE数据-->
	  <!--层次 和组合列表没有添加 李娟 11.11.10-->
	   <图:图形_8062>
          ...
          <!--liuyangyang，2015-03-06，添加leftCircularArrow翻转为CircularArrow-->
      ...
      <xsl:if test="p:spPr/a:prstGeom/a:avLst">        
  ...
  <xsl:template name="spPr">
    <!--xsl:if test="a:prstGeom"-->
     ...
     <!--liuyangyang 2015-03-06 添加blockarc旋转，修复控制点差异引起的图形不一致-->
     ...
     <!--2014-02-27, tangjiang, 修改OOXML到UOF图形背景颜色透明度BUG start -->
        <xsl:choose>
          <xsl:when test="a:grpFill">            
            <xsl:if test="ancestor::p:grpSp/p:grpSpPr//a:alpha or ancestor::p:grpSp/p:grpSpPr//a:alphaModFix">
              ...
              <xsl:choose>
                <xsl:when test="ancestor::p:grpSp/p:grpSpPr//a:alpha">
                  <xsl:variable name="alphaVal" select="round(100 - (ancestor::p:grpSp/p:grpSpPr//a:alpha/@val div 1000))"/>
                  <xsl:choose>
                ...
                <xsl:when test="ancestor::p:grpSp/p:grpSpPr//a:alphaModFix">
                  <xsl:variable name="alphaModFix" select="round(100 - (ancestor::p:grpSp/p:grpSpPr//a:alphaModFix/@amt div 1000))"/>
                  <xsl:choose>
                    <xsl:when test="$alphaModFix &lt; 0">
                      <xsl:value-of select="'0'"/>
                    </xsl:when>
                    <xsl:otherwise>
                      <xsl:value-of select="$alphaModFix"/>
                    </xsl:otherwise>
                  </xsl:choose>
                </xsl:when>
              </xsl:choose>
              ...
              <图:透明度_8050>
                <xsl:choose>
                  <xsl:when test="a:solidFill//a:alpha">
                    <xsl:variable name="alpha" select="round(100 - (a:solidFill//a:alpha/@val div 1000))"/>
                    <xsl:choose>
                      <xsl:when test="$alpha &lt; 0">
                        <xsl:value-of select="'0'"/>
                      </xsl:when>
                      <xsl:otherwise>
                  ...
                  <xsl:when test="a:gradFill//a:alpha">
                    <xsl:variable name="alpha" select="round(100 - (a:gradFill//a:alpha/@val div 1000))"/>
                    <xsl:choose>
                      <xsl:when test="$alpha &lt; 0">
                        <xsl:value-of select="'0'"/>
                      </xsl:when>
                  ...
                  <xsl:when test="a:pattFill//a:alpha">
                    <xsl:variable name="alpha" select="round(100 - (a:pattFill//a:alpha/@val div 1000))"/>
                    <xsl:choose>
                      <xsl:when test="$alpha &lt; 0">
                        <xsl:value-of select="'0'"/>
                      </xsl:when>
                  ...
                  <xsl:when test="a:blipFill//a:alphaModFix">
                    <xsl:variable name="alphaModFix" select="round(100 - (a:blipFill//a:alphaModFix/@amt div 1000))"/>
                    <xsl:choose>
                      <xsl:when test="$alphaModFix &lt; 0">
                        <xsl:value-of select="'0'"/>
                      </xsl:when>
              ...
              <图:透明度_8050>
                <xsl:choose>
                  <xsl:when test="a:solidFill//a:lumOff">
                    <xsl:variable name="solidFillOffVal" select="(a:solidFill//a:lumOff/@val div 1000)" />
                    <xsl:choose>
                      <xsl:when test="$solidFillOffVal &lt; 0 or $solidFillOffVal = 0">
                        <xsl:value-of select="'0'"/>
                      </xsl:when>
                      <xsl:otherwise>
                        <xsl:value-of select="round(100-$solidFillOffVal)"/>
                      </xsl:otherwise>
                    </xsl:choose>
                  </xsl:when>
                  <xsl:otherwise>
                    <xsl:variable name="solidFillModVal" select="(a:solidFill//a:lumMod/@val div 1000)" />
                    <xsl:choose>
                      <xsl:when test="$solidFillModVal &lt; 0 or $solidFillModVal = 0">
                        <xsl:value-of select="'0'"/>
                      </xsl:when>
                      <xsl:otherwise>
                        <xsl:value-of select="round(100-$solidFillModVal)"/>
                      </xsl:otherwise>
                    </xsl:choose>
                  </xsl:otherwise>
                </xsl:choose>
     ...
     <!--end 2014-02-27, tangjiang, 修改OOXML到UOF图形背景颜色透明度BUG -->
                ...
                <图:上_8023>
                  <xsl:choose>
                    <xsl:when test="parent::p:pic/p:blipFill/a:srcRect/@t">
                      <xsl:value-of select ="(parent::p:pic/p:blipFill/a:srcRect/@t) div 1000"/>
                    </xsl:when>
                    <xsl:otherwise>
                      <xsl:value-of select ="'0'"/>
                    </xsl:otherwise>
                ...
                <图:下_8024>
                  <xsl:choose>
                    <xsl:when test="parent::p:pic/p:blipFill/a:srcRect/@b">
                      <xsl:value-of select ="(parent::p:pic/p:blipFill/a:srcRect/@b) div 1000"/>
                    </xsl:when>
                    <xsl:otherwise>
                      <xsl:value-of select ="'0'"/>
                    </xsl:otherwise>
                ...
                <图:左_8025>
                  <xsl:choose>
                    <xsl:when test="parent::p:pic/p:blipFill/a:srcRect/@l">
                      <xsl:value-of select ="(parent::p:pic/p:blipFill/a:srcRect/@l) div 1000"/>
                    </xsl:when>
                    <xsl:otherwise>
                      <xsl:value-of select ="'0'"/>
                    </xsl:otherwise>
                ...
                <图:右_8026>
                  <xsl:choose>
                    <xsl:when test="parent::p:pic/p:blipFill/a:srcRect/@r">
                      <xsl:value-of select ="(parent::p:pic/p:blipFill/a:srcRect/@r) div 1000"/>
                    </xsl:when>
                    <xsl:otherwise>
                      <xsl:value-of select ="'0'"/>
                    </xsl:otherwise>
  ...
  <xsl:template name="shdwCommen">
    <xsl:if test="a:effectLst/a:outerShdw|ancestor::p:grpSp/p:grpSpPr/a:effectLst/a:outerShdw">
      ...
      <xsl:choose>
        ...
        <xsl:when test="a:effectLst/a:outerShdw//a:alpha">
          <xsl:attribute name="透明度_C61F">
            <xsl:value-of select="round(a:effectLst/a:outerShdw//a:alpha/@val div 1000)"/>
          </xsl:attribute>
        </xsl:when>
        <xsl:when test="ancestor::p:grpSp/p:grpSpPr/a:effectLst/a:outerShdw//a:alpha">
          <xsl:attribute name="透明度_C61F">
            <xsl:value-of select="round(ancestor::p:grpSp/p:grpSpPr/a:effectLst/a:outerShdw//a:alpha/@val div 1000)"/>
          </xsl:attribute>
        </xsl:when>
        ...
        <xsl:choose>
          <xsl:when test="a:effectLst/a:outerShdw/x">
            ...
            <xsl:choose>
              <!-- 负指数已经很小了，按0值处理 -->
              <xsl:when test="contains($xVal,'E-05')">
                <xsl:attribute name="x_C606">
                  <xsl:value-of select="'0'"/>
                </xsl:attribute>
              </xsl:when>
        ...
        <xsl:choose>
          <xsl:when test="a:effectLst/a:outerShdw/y">
            ...
            <xsl:choose>
              <!-- 负指数已经很小了，按0值处理 -->
              <xsl:when test="contains($yVal,'E-05')">
                <xsl:attribute name="y_C607">
                  <xsl:value-of select="'0'"/>
                </xsl:attribute>
              </xsl:when>
...
<!--
  ...
  <xsl:template match="a:bodyPr">
    ...
    <!--2014-03-09, pengxin, 修改OOXML到UOF图形文本框过大的错误和图形内部文本中边距转换有误 start -->
    <图:边距_803D>
      <xsl:choose>
        <xsl:when test="./@lIns">
