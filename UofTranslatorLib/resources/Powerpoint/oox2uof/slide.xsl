...
<xsl:stylesheet version="1.0"
    ...
    <!--2010-11-8 罗文甜：删除幻灯片下面的“配色方案引用”属性-->
        ...
        <!--liuyangyang 2015-03-06 修改幻灯片编号，与永中生成编号前置相同-->
      ...
      -->
			<xsl:if test="following-sibling::rel:Relationships[1]/rel:Relationship[@Type='http://schemas.openxmlformats.org/officeDocument/2006/relationships/notesSlide']">
				<xsl:apply-templates select="following-sibling::p:notes[1]" mode="note"/>
			</xsl:if>
			<!--切换2012-4-24李娟，修改-->
	...
	<xsl:template match="p:notes" mode="note">
		<演:幻灯片备注_6B1D>
			<xsl:attribute name="备注母版引用_6B2D">
				<xsl:value-of select="substring-before(substring-after(following-sibling::rel:Relationships[1]/rel:Relationship[@Type='http://schemas.openxmlformats.org/officeDocument/2006/relationships/notesMaster']/@Target,'notesMasters/'),'.xml')"/>
			</xsl:attribute>
