...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template name="chart-biankuang">
    ...
    <xsl:for-each select="a:ln">
      <图表:边框线_4226>
        ...
        <!--颜色-->
        <xsl:if test="not(a:solidFill/a:srgbClr or a:solidFill/a:schemeClr)">
  ...
  <xsl:template name="table-tbziti">
    ...
    <xsl:choose>
      ...
      <xsl:when test="a:defRPr[@baseline]">
        <字:上下标类型_4143>
          <xsl:choose>
            <xsl:when test="a:defRPr[number(substring-before(@baseline, '%')) = 0]">
              <xsl:value-of select="'none'"/>
            </xsl:when>
            <xsl:when test="a:defRPr[number(substring-before(@baseline, '%')) &gt;0]">
      ...
      <xsl:otherwise>
        <xsl:if test="./ancestor::c:chartSpace/c:txPr/a:p/a:pPr/a:defRPr[@baseline]">
          <字:上下标类型_4143>
            <xsl:choose>
              <xsl:when test="./ancestor::c:chartSpace/c:txPr/a:p/a:pPr/a:defRPr[number(substring-before(@baseline, '%')) = 0]">
                <xsl:value-of select="'none'"/>
              </xsl:when>
              <xsl:when test="./ancestor::c:chartSpace/c:txPr/a:p/a:pPr/a:defRPr[number(substring-before(@baseline, '%')) &gt;0]">
                <xsl:value-of select="'sup'"/>
      ...
      <xsl:when test="a:solidFill/a:schemeClr">
        ...
        <xsl:choose>
          ...
          <xsl:when test="$id='accent4'">
            ...
            <!--2014-5-31, update by Qihy,bug3377中的用例“011图表.xlsx”互操作后打开需要恢复， start -->
            <!--<xsl:value-of select="'#C0C0C0z'"/>-->
  ...
  <xsl:template match="c:ser">
    ...
    <xsl:if test="name()='c:ser'">
      ...
      <图表:数据系列_E74F>
        ...
        <xsl:if test="./c:xVal/c:strRef or ./c:cat/c:strRef or ./c:cat/c:numRef">
        ...
        <xsl:variable name="childType">
          <xsl:choose>
            ...
            <!--散点图子类型-->
            <xsl:when test="$chart_type='c:scatterChart' and ancestor-or-self::ws:spreadsheet/ws:Drawings/xdr:wsDr/c:chartSpace[@filename=substring-after($tar,'charts/')]/c:chart/c:plotArea/c:scatterChart[c:scatterStyle/@val='lineMarker' and c:ser[c:spPr]]">
  ...
  <xsl:template name="caculateValue">
    ...
    <xsl:variable name="name">
      <xsl:choose>
        ...
        <xsl:when test="contains($gValue,'(Income')">
          <!--zl 2015-5-5-->
          <xsl:value-of select="substring-before(substring-after($gValue,'('),')')"/>
          <!--zl 2015-5-5-->      
		    </xsl:when>

        <xsl:when test="contains($gValue,'(Expenses')">
          <!--zl 2015-5-5-->
          <xsl:value-of select="substring-before(substring-after($gValue,'('),')')"/>
          <!--zl 2015-5-5-->
        </xsl:when>
  ...
  <xsl:template name="DisplayLabel">
    <图表:数据标签_E752>
      ...
      <xsl:if test="not(c:showLegendKey) or c:showLegendKey[@val='1']">
        <xsl:attribute name="是否显示图例标志_E719">
          <xsl:value-of select="'false'"/>
        </xsl:attribute>
      </xsl:if>
    ...
    <图表:图例_E794>
      ...
      <xsl:if test="not(ancestor-or-self::ws:spreadsheet/ws:Drawings/xdr:wsDr/c:chartSpace[@filename=substring-after($target,'charts/')]/c:chart/c:legend/c:legendPos/@val='r' and ancestor-or-self::ws:spreadsheet/ws:Drawings/xdr:wsDr/c:chartSpace[@filename=substring-after($target,'charts/')]/c:chart[c:legend/c:layout/c:manualLayout])">
        ...
        <xsl:if test="ancestor-or-self::ws:spreadsheet/ws:Drawings/xdr:wsDr/c:chartSpace[@filename=substring-after($target,'charts/')]/c:chart[c:legend/c:legendPos]">
          <xsl:for-each select="ancestor-or-self::ws:spreadsheet/ws:Drawings/xdr:wsDr/c:chartSpace[@filename=substring-after($target,'charts/')]/c:chart/c:legend/c:legendPos">
            <图表:图例位置_E795>
              <xsl:if test="./@val='r'">
  ...
  <xsl:template name="table-btj-ziti">
    ...
    <xsl:choose>
      <xsl:when test="a:rPr[@baseline]">
        <字:上下标类型_4143>
          <xsl:choose>
            <xsl:when test="a:rPr[number(substring-before(@baseline, '%')) = 0]">
              <xsl:value-of select="'none'"/>
            </xsl:when>
            <xsl:when test="a:rPr[number(substring-before(@baseline, '%')) &gt;0]">
              <xsl:value-of select="'sup'"/>
      ...
      <xsl:when test="not(a:rPr[number(substring-before(@baseline, '%'))]) and ./preceding-sibling::a:pPr/a:defRPr[@baseline]">
        <字:上下标类型_4143>
          <xsl:choose>
            <xsl:when test=" ./preceding-sibling::a:pPr/a:defRPr[number(substring-before(@baseline, '%')) = 0]">
              <xsl:value-of select="'none'"/>
            </xsl:when>
            <xsl:when test=" ./preceding-sibling::a:pPr/a:defRPr[number(substring-before(@baseline, '%')) &gt;0]">
              <xsl:value-of select="'sup'"/>
      ...
      <xsl:for-each select="./a:p/a:pPr/a:defRPr">
        <图表:字体_E70B>
          <字:字体_4128>
            ...
            <!--2014-6-9, update by Qihy， 图表字号取值不正确， start-->
          ...
          <xsl:choose>
            <xsl:when test="./@baseline">
              <字:上下标类型_4143>
                <xsl:choose>
                  <xsl:when test="number(substring-before(@baseline, '%')) = '0'">
                    <xsl:value-of select="'none'"/>
                  </xsl:when>
                  <xsl:when test="number(substring-before(@baseline, '%')) &gt; 0">
                    <xsl:value-of select="'sup'"/>
