...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template name ="fh">
    ...
    <xsl:choose >
      ...
      <xsl:when test="contains($v2,'A')">
        <字:域开始_419E 类型_416E="sheetName" 是否锁定_416F="false"/>
        <字:域代码_419F>
          <字:段落_416B>
            <字:句_419D>
              <字:句属性_4158/>
        ...
        <字:域代码_419F>
          <字:段落_416B>
            ...
            <字:句_419D>
              ...
              <字:空格符_4161/>
              <字:文本串_415B>"yyyy-M-d"</字:文本串_415B>
            </字:句_419D>
          </字:段落_416B>
        </字:域代码_419F>
        <字:域结束_41A0/>
  ...
  <xsl:template match="ws:headerFooter">
              ...
              <xsl:variable name ="substr">
                <xsl:choose>
                  ...
                  <xsl:when test ="contains($text,'&amp;N')">
                    <xsl:value-of select ="translate($text,'&amp;N','')"/>
                  </xsl:when>
    ...
    <表:视图_E7D5>
      ...
      <!--2013-11-26 Qihy 增加网格线颜色设置的实现 start-->
      <!--<xsl:if test ="ws:sheetView[@defaultGridColor='0']">
        <表:网格颜色_E7E0>#00ccff</表:网格颜色_E7E0>
      </xsl:if>-->
      <xsl:if test ="ws:sheetView[@defaultGridColor='0']">
        <表:网格颜色_E7E0>
          <xsl:choose>
