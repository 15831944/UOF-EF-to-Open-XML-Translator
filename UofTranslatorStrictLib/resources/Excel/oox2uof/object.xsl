...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:ws="http://purl.oclc.org/ooxml/spreadsheetml/main" xmlns:r="http://purl.oclc.org/ooxml/officeDocument/relationships" xmlns:a="http://purl.oclc.org/ooxml/drawingml/main" xmlns:c="http://purl.oclc.org/ooxml/drawingml/chart" xmlns:p="http://purl.oclc.org/ooxml/presentationml/main" xmlns:ori="http://purl.oclc.org/ooxml/officeDocument/relationships/image" xmlns:dgm="http://purl.oclc.org/ooxml/drawingml/diagram" xmlns:mc="http://schemas.openxmlformats.org/markup-compatibility/2006" xmlns:pr="http://schemas.openxmlformats.org/package/2006/relationships" xmlns:图="http://schemas.uof.org/cn/2009/graph" xmlns:图形="http://schemas.uof.org/cn/2009/graphics" xmlns:uof="http://schemas.uof.org/cn/2009/uof" xmlns:字="http://schemas.uof.org/cn/2009/wordproc" xmlns:对象="http://schemas.uof.org/cn/2009/objects" xmlns:式样="http://schemas.uof.org/cn/2009/styles" xmlns:xdr="http://purl.oclc.org/ooxml/drawingml/spreadsheetDrawing" xmlns:dsp="http://schemas.microsoft.com/office/drawing/2008/diagram">
	...
	<xsl:template name="object_root">
		...
		<!--2013-11-06 Qihy 修改type的命名空间，改为相应strict的命名空间确保工作表设置中的背景功能点正确转换 start-->
		<xsl:for-each select=" .//ws:spreadsheets/ws:spreadsheet/pr:Relationships/pr:Relationship[@Type='http://purl.oclc.org/ooxml/officeDocument/relationships/image']">
			<!--2013-11-06 Qihy 修改type的命名空间，改为相应strict的命名空间确保工作表设置中的背景功能点正确转换 end-->
			...
			<对象:对象数据_D701>
				<!--标识符:image1-->
				<xsl:attribute name="标识符_D704"><xsl:value-of select="$imageName"/></xsl:attribute>
				<xsl:attribute name="是否内嵌_D705">false</xsl:attribute>
				<xsl:choose>
					<xsl:when test="contains($target,'emf')">
						<xsl:attribute name="私有类型_D707"><xsl:variable name="type"><xsl:value-of select="substring-after($target,'../media/')"/></xsl:variable><xsl:value-of select="substring-after($type,'.')"/></xsl:attribute>
					</xsl:when>
					<xsl:otherwise>
						<xsl:attribute name="公共类型_D706"><xsl:variable name="type"><xsl:value-of select="substring-after($target,'../media/')"/></xsl:variable><xsl:value-of select="substring-after($type,'.')"/></xsl:attribute>
					</xsl:otherwise>
				</xsl:choose>
				<!--<对象:数据_D702>
          -->
		...
		<xsl:for-each select="./ws:spreadsheets/ws:spreadsheet/ws:Drawings/xdr:wsDr/pr:Relationships/pr:Relationship[@Type='http://purl.oclc.org/ooxml/officeDocument/relationships/image']">
			<xsl:variable name="rID">
				<xsl:value-of select="./@Id"/>
			</xsl:variable>
			<xsl:variable name="target">
		...
		<xsl:for-each select="./ws:spreadsheets/ws:spreadsheet/ws:Drawings/xdr:wsDr/c:chartSpace/pr:Relationships/pr:Relationship[@Type='http://purl.oclc.org/ooxml/officeDocument/relationships/image']">
			<xsl:variable name="target">
				<xsl:value-of select="./@Target"/>
			</xsl:variable>
			<xsl:variable name="imageName">
	...
	<xsl:template match="xdr:cNvPr">
		...
		<图:图形_8062>
			...
			<xsl:attribute name="标识符_804B"><xsl:value-of select="concat($s_name,'_OBJ0000',$ppid)"/></xsl:attribute>
			<图:图表数据引用_8065>
	...
	<xsl:template match="xdr:pic">
		...
		<图:图形_8062>
          ...
          <xsl:value-of select="xdr:nvCxnSpPr/xdr:cNvPr/@id"/>
			...
			<!--3yue27ri 夏艳霞checked-->
			<xsl:attribute name="标识符_804B"><xsl:value-of select="concat($s_name,'_OBJ0000',$ppid)"/></xsl:attribute>
			<xsl:attribute name="层次_8063"><xsl:number count="xdr:twoCellAnchor" level="single"/></xsl:attribute>
	...
	<xsl:template name="pizhuji-root">
		...
		<xsl:variable name="sheetId" select="./ancestor::ws:spreadsheet/@sheetId"/>
		<xsl:for-each select=".//ws:comment">
			<xsl:variable name="p">
				<xsl:number count="ws:comment" level="single"/>
			</xsl:variable>
			<图:图形_8062>
				<xsl:attribute name="标识符_804B"><xsl:value-of select="concat($a,$p)"/><!--标识符_804B:comments1.1--></xsl:attribute>
				...
				<!--文本内容-->
				<图:文本_803C 是否为文本框_8046="false" 是否自动换行_8047="true" 是否大小适应文字_8048="false" 是否文字随边框自动缩放_804A="true" 是否文字绕排外部对象_8068="true">
					<!--2012-02-24, Qihy, add 解决OOXML到UOF批注 边距效果设置失效问题,修复BUG3008 start-->
					<!--<图:边距_803D 左_C608="7.2" 上_C609="3.6" 右_C60A="7.2" 下_C60B="3.6"/>-->
					<xsl:if test="ancestor::ws:spreadsheet[@sheetId = $sheetId]/ws:Drawings/xdr:wsDr/mc:AlternateContent[position() = $p]/mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:txBody/a:bodyPr">
						<图:边距_803D>
							<xsl:if test="ancestor::ws:spreadsheet[@sheetId = $sheetId]/ws:Drawings/xdr:wsDr/mc:AlternateContent[position() = $p]/mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:txBody/a:bodyPr/@lIns">
								<xsl:attribute name="左_C608"><xsl:value-of select="ancestor::ws:spreadsheet[@sheetId = $sheetId]/ws:Drawings/xdr:wsDr/mc:AlternateContent[position() = $p]/mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:txBody/a:bodyPr/@lIns div 12700"/></xsl:attribute>
							</xsl:if>
							<xsl:if test="ancestor::ws:spreadsheet[@sheetId = $sheetId]/ws:Drawings/xdr:wsDr/mc:AlternateContent[position() = $p]/mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:txBody/a:bodyPr/@tIns">
								<xsl:attribute name="上_C609"><xsl:value-of select="ancestor::ws:spreadsheet[@sheetId = $sheetId]/ws:Drawings/xdr:wsDr/mc:AlternateContent[position() = $p]/mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:txBody/a:bodyPr/@tIns div 12700"/></xsl:attribute>
							</xsl:if>
							<xsl:if test="ancestor::ws:spreadsheet[@sheetId = $sheetId]/ws:Drawings/xdr:wsDr/mc:AlternateContent[position() = $p]/mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:txBody/a:bodyPr/@rIns">
								<xsl:attribute name="右_C60A"><xsl:value-of select="ancestor::ws:spreadsheet[@sheetId = $sheetId]/ws:Drawings/xdr:wsDr/mc:AlternateContent[position() = $p]/mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:txBody/a:bodyPr/@rIns div 12700"/></xsl:attribute>
							</xsl:if>
							<xsl:if test="ancestor::ws:spreadsheet[@sheetId = $sheetId]/ws:Drawings/xdr:wsDr/mc:AlternateContent[position() = $p]/mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:txBody/a:bodyPr/@bIns">
								<xsl:attribute name="下_C60B"><xsl:value-of select="ancestor::ws:spreadsheet[@sheetId = $sheetId]/ws:Drawings/xdr:wsDr/mc:AlternateContent[position() = $p]/mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:txBody/a:bodyPr/@bIns div 12700"/></xsl:attribute>
							</xsl:if>
						</图:边距_803D>
					</xsl:if>
					<!--end-->
					<!--2014-2-24, Qihy, update, 增加批注中的文本对齐方式的转换(原来为固定值)，注：UOF标准支持,但是永中软件不支持，转换后效果丢失 start-->
					<!--<图:对齐_803E 水平对齐_421D="left" 文字对齐_421E="top"/>-->
					<xsl:variable name="align">
						<xsl:choose>
							<xsl:when test=".//parent::ws:comment//ws:commentPr/@textHAlign">
								<xsl:value-of select=".//parent::ws:comment//ws:commentPr/@textHAlign"/>
							</xsl:when>
							<xsl:otherwise>
					...
					<图:对齐_803E>
						...
						<xsl:attribute name="文字对齐_421E"><xsl:choose><xsl:when test=".//parent::ws:comment//ws:commentPr/@textVAlign"><xsl:value-of select=".//parent::ws:comment//ws:commentPr/@textVAlign"/></xsl:when><xsl:otherwise><xsl:value-of select="'top'"/></xsl:otherwise></xsl:choose></xsl:attribute>
					</图:对齐_803E>
					<!--end-->
	...
	<xsl:template name="ydy-tuxing">
		...
		<图:属性_801D>
			<!--2012-02-24, Qihy, update 解决OOXML到UOF批注边框样式丢失BUG3002和批注填充BUG3007问题  start-->
        ...
        -->
			...
			<xsl:for-each select="ancestor::ws:spreadsheet[@sheetId = $sheetId]/ws:Drawings/xdr:wsDr/mc:AlternateContent[position() = $pos]">
				<!--2014-2-25, Qihy, update, 修改填充，增加包括颜色，渐变色，图案，纹理，图形填充，透明度  start-->
				<xsl:if test="mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:solidFill/a:srgbClr/a:alpha">
					<图:透明度_8050>
						<xsl:value-of select="round(100 - number(substring-before(mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:solidFill/a:srgbClr/a:alpha/@val, '%')))"/>
					</图:透明度_8050>
				</xsl:if>
				<图:填充_804C 是否填充随图形旋转_8067="false">
					<xsl:choose>
						<xsl:when test="mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:pattFill">

							<图:图案_800A>
								<xsl:if test="mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:pattFill/@prst">
									<xsl:variable name="type">
										<xsl:value-of select="mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:pattFill/@prst"/>
									</xsl:variable>
									<xsl:attribute name="类型_8008"><xsl:call-template name="ttype"><xsl:with-param name="tttype" select="$type"/></xsl:call-template></xsl:attribute>
								</xsl:if>
								<!--图形引用属性-->
								<xsl:if test="mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:pattFill/a:fgClr or mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:pattFill/a:bgClr">
									<xsl:for-each select="mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:pattFill/a:fgClr">
										<xsl:attribute name="前景色_800B"><xsl:call-template name="pattClr"/></xsl:attribute>
									</xsl:for-each>
									<xsl:for-each select="mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:pattFill/a:bgClr">
										<xsl:attribute name="背景色_800C"><xsl:call-template name="pattClr"/></xsl:attribute>
									</xsl:for-each>
								</xsl:if>
							</图:图案_800A>
						</xsl:when>
						<xsl:when test="mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:solidFill/a:srgbClr">

							<图:颜色_8004>
								<xsl:value-of select="concat('#',mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:solidFill/a:srgbClr/@val)"/>
							</图:颜色_8004>
						</xsl:when>
						<xsl:when test="mc:Fallback/xdr:twoCellAnchor/xdr:sp/preceding-sibling::xdr:grpSpPr/a:solidFill">

							<图:颜色_8004>
								<!-- update by xuzhenwei BUG_2727: 集成OO-UOF 预定义图形大小不一致；填充，边框转换前后不一致；三维效果丢失；部分文本框对齐方式转换前后不一致。-->
								<xsl:choose>
									<xsl:when test="mc:Fallback/xdr:twoCellAnchor/xdr:sp/preceding-sibling::xdr:grpSpPr/a:solidFill/a:srgbClr">
										<xsl:value-of select="concat('#',mc:Fallback/xdr:twoCellAnchor/xdr:sp/preceding-sibling::xdr:grpSpPr/a:solidFill/a:srgbClr/@val)"/>
									</xsl:when>
									<xsl:otherwise>
										<xsl:variable name="schemeid" select="mc:Fallback/xdr:twoCellAnchor/xdr:sp/preceding-sibling::xdr:grpSpPr/a:solidFill/a:schemeClr/@val"/>
										<xsl:choose>
						...
						<xsl:when test="mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:solidFill/a:schemeClr">
           
							<xsl:variable name="id">
								<xsl:value-of select="mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:solidFill/a:schemeClr/@val"/>
							</xsl:variable>
							<图:颜色_8004>
								<xsl:choose>
						...
						<xsl:when test="mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:gradFill">
  
							<!--2014-4-9, update by Qihy, 批注填充，单色渐变和双色渐变同时存在时，无法进行转换， start-->
							<!--<xsl:for-each select ="mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr">
                <xsl:apply-templates select="a:gradFill"/>
              </xsl:for-each>-->
							<xsl:apply-templates select="mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:gradFill"/>
							<!--2014-4-9 end-->
						</xsl:when>
						<xsl:when test="mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:blipFill">
            
							<xsl:variable name="reference">
								<xsl:value-of select="mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:blipFill/a:blip/@ r:embed"/>
							</xsl:variable>
							<xsl:variable name="picRefer">
								<xsl:value-of select="substring-before(substring-after(ancestor::ws:spreadsheet[@sheetId = $sheetId]/ws:Drawings/xdr:wsDr/pr:Relationships/pr:Relationship[@Id = $reference]/@Target,'media/'),'.')"/>
							</xsl:variable>
							<图:图片_8005>
								<xsl:if test="mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:blipFill/a:stretch">
									<xsl:attribute name="位置_8006"><xsl:value-of select="'stretch'"/></xsl:attribute>
								</xsl:if>
								<xsl:attribute name="图形引用_8007"><xsl:value-of select="$picRefer"/></xsl:attribute>
								<!--图片的 类型，名称 属性-->
							</图:图片_8005>
						</xsl:when>
						<xsl:when test="mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:noFill">

							<图:颜色_8004>
								<xsl:value-of select="'auto'"/>
							</图:颜色_8004>
						</xsl:when>
						<xsl:when test="mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:custGeom">
           
							<xsl:if test="mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:custGeom/a:pathLst/a:path/a:moveTo/a:pt/@x = mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:custGeom/a:pathLst/a:path/a:lnTo[position()=last()]/a:pt/@x and mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:custGeom/a:pathLst/a:path/a:moveTo/a:pt/@y = mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:custGeom/a:pathLst/a:path/a:lnTo[position()=last()]/a:pt/@y">
								<xsl:if test="not(mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:solidFill) and not(mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:pattFill)">
									<图:颜色_8004>
										<xsl:value-of select="'#4F81BD'"/>
									</图:颜色_8004>
								</xsl:if>
						...
						<xsl:when test="not(mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:solidFill or mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:gradFill) and mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/following-sibling::xdr:style and a:prstGeom/@prst!='bracketPair' and mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:prstGeom/@prst!='bracePair' and mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:prstGeom/@prst!='leftBracket' and mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:prstGeom/@prst!='rightBracket' and mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:prstGeom/@prst!='leftBrace' and mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:prstGeom/@prst!='rightBrace'">
              
              <xsl:choose>
								<xsl:when test="mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/following-sibling::xdr:style/a:fillRef/a:schemeClr">
									<xsl:variable name="id">
										<xsl:value-of select="mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/following-sibling::xdr:style/a:fillRef/a:schemeClr/@val"/>
									</xsl:variable>
									<图:颜色_8004>
										<xsl:choose>
											<xsl:when test="$id='dk1'">
								...
								<xsl:when test="mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/following-sibling::xdr:style/a:fillRef/a:srgbClr">
									<图:颜色_8004>
										<xsl:value-of select="concat('#',mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/following-sibling::xdr:style/a:fillRef/a:srgbClr/@val)"/>
									</图:颜色_8004>
								</xsl:when>
							</xsl:choose>
						</xsl:when>
				...
				<xsl:variable name="lineColor" select="concat('#',mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:ln/a:solidFill/a:srgbClr/@val)"/>
				<图:线_8057>
					<图:线颜色_8058>
						<xsl:value-of select="$lineColor"/>
					</图:线颜色_8058>
					<图:线类型_8059>
						<xsl:attribute name="线型_805A"><xsl:value-of select="'single'"/></xsl:attribute>
						<xsl:attribute name="虚实_805B"><xsl:choose><xsl:when test="mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:ln/a:prstDash/@val='solid'"><xsl:value-of select="'solid'"/></xsl:when><xsl:when test="mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:ln/a:prstDash/@val='dot'"><xsl:value-of select="'square-dot'"/></xsl:when><xsl:when test="mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:ln/a:prstDash/@val='dash'"><xsl:value-of select="'dash'"/></xsl:when><xsl:when test="mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:ln/a:prstDash/@val='lgDash'"><xsl:value-of select="'long-dash'"/></xsl:when><xsl:when test="mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:ln/a:prstDash/@val='dashDot'"><xsl:value-of select="'dash-dot'"/></xsl:when><xsl:when test="mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:ln/a:prstDash/@val='lgDashDot'"><xsl:value-of select="'dash-dot'"/></xsl:when><xsl:when test="mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:ln/a:prstDash/@val='lgDashDotDot'"><xsl:value-of select="'dash-dot-dot'"/></xsl:when><xsl:when test="mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:ln/a:prstDash/@val='sysDash'"><xsl:value-of select="'dash'"/></xsl:when><xsl:when test="mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:ln/a:prstDash/@val='sysDot'"><xsl:value-of select="'square-dot'"/></xsl:when><xsl:when test="mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:ln/a:prstDash/@val='sysDashDot'"><xsl:value-of select="'dash-dot'"/></xsl:when><xsl:when test="mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:ln/a:prstDash/@val='sysDashDotDot'"><xsl:value-of select="'dash-dot-dot'"/></xsl:when></xsl:choose></xsl:attribute>
					</图:线类型_8059>
					<图:线粗细_805C>
						<xsl:value-of select="mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:ln/@w div 12700"/>
					</图:线粗细_805C>
				</图:线_8057>
				<!--2014-2-24, Qihy, update, 解决BUG3003-批注大小设置部分失效问题 start-->
				<图:大小_8060>
					<xsl:attribute name="长_C604"><xsl:choose><xsl:when test="mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:xfrm"><xsl:value-of select="mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:xfrm/a:ext/@cy div 12700"/></xsl:when><xsl:otherwise><xsl:value-of select="59.999084"/></xsl:otherwise></xsl:choose></xsl:attribute>
					<xsl:attribute name="宽_C605"><xsl:choose><xsl:when test="mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:xfrm"><xsl:value-of select="mc:Fallback/xdr:twoCellAnchor/xdr:sp/xdr:spPr/a:xfrm/a:ext/@cx div 12700"/></xsl:when><xsl:otherwise><xsl:value-of select="97.49851"/></xsl:otherwise></xsl:choose></xsl:attribute>
				</图:大小_8060>
				<!--end-->
			</xsl:for-each>
			<!--end-->
	...
	<xsl:template name="wbnr">
		...
		<xsl:param name="cname"/>
		<xsl:param name="pos"/>
		<!--2014-02-26, Qihy, update, 更新文本对齐方式的转换（原来固定为靠左对齐））-->
		...
		<xsl:for-each select="./ancestor-or-self::ws:spreadsheet/ws:comments/ws:commentList/ws:comment[position()=$pos]/ws:text">
			...
			<图:内容_8043>
				<字:段落_416B>
					<字:段落属性_419B>
						<字:对齐_417D>
							<xsl:attribute name="字:水平对齐"><xsl:value-of select="$align"/></xsl:attribute>
						</字:对齐_417D>
              ...
              <uof:下_C616 线型_C60D="none"/>
					...
					<xsl:for-each select="ws:r">
							...
							<字:句属性_4158>
								<xsl:if test="ws:rPr/ws:rFont">
									<字:字体_4128>
										...
										<xsl:if test="ws:rPr[ws:rFont]">
											<xsl:attribute name="西文字体引用_4129"><xsl:value-of select="concat('font_',$fontid)"/></xsl:attribute>
											<xsl:attribute name="西文字体引用_4129"><xsl:value-of select="concat('font_',$fontid)"/></xsl:attribute>
										</xsl:if>
										<!--字号 -->
										<xsl:if test="ws:rPr[ws:sz]">
											<xsl:attribute name="字号_412D"><xsl:value-of select="ws:rPr/ws:sz/@val"/></xsl:attribute>
										</xsl:if>
										<!--color-->
                      ...
                      <xsl:variable name="them2" select="document('xlsx/xl/theme/theme1.xml')/a:theme/a:themeElements/a:clrScheme/*[position()=$the]/a:srgbClr/@val"/>-->
												...
												<xsl:attribute name="颜色_412F"><xsl:choose><xsl:when test="$them='windowText'"><xsl:value-of select="'#000000'"/></xsl:when><xsl:when test="$them='window'"><xsl:value-of select="'#000000'"/></xsl:when><xsl:otherwise><xsl:variable name="colll"><xsl:value-of select="translate($them2,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz')"/></xsl:variable><xsl:value-of select="concat('#',$colll)"/></xsl:otherwise></xsl:choose></xsl:attribute>
											</xsl:if>
											<xsl:if test="ws:rPr/ws:color[@rgb]">
												<xsl:attribute name="颜色_412F"><xsl:variable name="c"><xsl:value-of select="ws:rPr/ws:color/@rgb"/></xsl:variable><xsl:variable name="co"><xsl:value-of select="substring($c,3,8)"/></xsl:variable><xsl:variable name="col"><xsl:value-of select="translate($co,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz')"/></xsl:variable><xsl:value-of select="concat('#',$col)"/></xsl:attribute>
											</xsl:if>
										</xsl:if>
									</字:字体_4128>
								</xsl:if>
								...
								<xsl:if test="ws:rPr[ws:u]">
									<xsl:if test="ws:rPr/ws:u[@val]">
										<字:下划线_4136>
											<xsl:attribute name="线型_4137"><xsl:if test="ws:rPr/ws:u[@val='double' or @val='doubleAccounting' ]"><xsl:value-of select="'double'"/></xsl:if><xsl:if test="ws:rPr/ws:u[@val='single' or @val='singleAccounting' ]"><xsl:value-of select="'single'"/></xsl:if></xsl:attribute>
										</字:下划线_4136>
									</xsl:if>
								</xsl:if>
	...
	<xsl:template name="ShapeClass2">
		<xsl:if test="a:prstGeom/@prst">
			<xsl:choose>
				...
				<xsl:when test="a:prstGeom/@prst='flowChartExtract'">322</xsl:when>
	...
	<xsl:template match="xdr:spPr|dsp:spPr" mode="second">
			...
			<xsl:when test="a:blipFill">
				...
				<图:填充_804C>
					<图:图片_8005>
						...
						<xsl:attribute name="图形引用_8007"><xsl:value-of select="substring-before($yuding3,'.')"/></xsl:attribute>
						<!--图片的 类型，名称 属性-->
					</图:图片_8005>
				</图:填充_804C>
			</xsl:when>
        ...
        <xsl:value-of select="number(100000-$tm) div 1000"/>-->
					<xsl:value-of select="substring-before(a:solidFill/*/a:alpha/@val,'%')"/>
				</xsl:variable>
				<xsl:value-of select="round(100 - number($tm))"/>
				<!--2014-3-13 end-->
			</图:透明度_8050>
		</xsl:if>
      ...
      <图:透明度_8050>
		...
		<xsl:if test="a:xfrm/a:ext">
			<图:大小_8060>
				<xsl:attribute name="长_C604"><!--修改图片高度（长）李杨 2012-5-24--><!--<xsl:value-of select="a:xfrm/a:ext/@cy div 12800"/>--><xsl:value-of select="a:xfrm/a:ext/@cy div 12700"/></xsl:attribute>
				<xsl:attribute name="宽_C605"><!-- 2012-11-27, xuzhenwei, 解决预定义图形的位置与原来不一致的BUG start --><!--<xsl:choose>--><!-- 判断预定义图形是否拉伸，暂时先通过width大于20来判断 --><!--<xsl:when test="ancestor::ws:spreadsheet/ws:worksheet/ws:cols/ws:col/@width &gt; 20">
            ...
            <xsl:otherwise>--><xsl:value-of select="a:xfrm/a:ext/@cx div 12700"/><!--</xsl:otherwise>
          </xsl:choose>--><!-- end --></xsl:attribute>
			</图:大小_8060>
		</xsl:if>
		<!--三维效果-->
			...
			<xsl:for-each select="a:ln[not(a:noFill)]">
				<图:线_8057>
						...
						<图:线颜色_8058>
							...
							<xsl:if test="a:solidFill[a:schemeClr]">
								...
								<xsl:choose>
									...
									<!-- 20130402 update by xuzhenwei BUG_2727 集成OO-UOF 预定义图形大小不一致；填充，边框转换前后不一致；三维效果丢失；部分文本框对齐方式转换前后不一致。start -->
									<xsl:when test="parent::xdr:spPr/a:sp3d/a:extrusionClr or parent::xdr:spPr/a:sp3d">
										<xsl:choose>
											<xsl:when test="parent::xdr:spPr/a:sp3d/a:contourClr/a:schemeClr/@val">
												<xsl:variable name="id1" select="parent::xdr:spPr/a:sp3d/a:contourClr/a:srgbClr/@val"/>
												<xsl:choose>
													<xsl:when test="$id1='dk2'">
														<xsl:value-of select="'#1F497D'"/>
													</xsl:when>
													<xsl:when test="$id1='lt2'">
														<xsl:value-of select="'#EEECE1'"/>
													</xsl:when>
													<xsl:when test="$id1='accent1'">
														<xsl:value-of select="'#4F81BD'"/>
													</xsl:when>
													<xsl:when test="$id1='accent2'">
														<xsl:value-of select="'#C0504D'"/>
													</xsl:when>
													<xsl:when test="$id1='accent3'">
														<xsl:value-of select="'#9BBB59'"/>
													</xsl:when>
													<xsl:when test="$id1='accent4'">
														<xsl:value-of select="'#C0C0C0'"/>
													</xsl:when>
													<xsl:when test="$id1='accent5'">
														<xsl:value-of select="'#4BACC6'"/>
													</xsl:when>
													<xsl:when test="$id1='accent6'">
														<xsl:value-of select="'#F79646'"/>
													</xsl:when>
													<xsl:when test="$id1='hlink'">
														<xsl:value-of select="'#0000FF'"/>
													</xsl:when>
													<xsl:when test="$id1='folHlink'">
														<xsl:value-of select="'#800080'"/>
													</xsl:when>
													<xsl:otherwise>
					...
					<xsl:if test="a:prstDash/@val and not(@cmpd)">
						<图:线类型_8059>
							<xsl:attribute name="线型_805A"><xsl:value-of select="'single'"/></xsl:attribute>
							<xsl:attribute name="虚实_805B"><xsl:choose><xsl:when test="a:prstDash/@val='solid'"><xsl:value-of select="'solid'"/></xsl:when><xsl:when test="a:prstDash/@val='dot'"><xsl:value-of select="'square-dot'"/></xsl:when><xsl:when test="a:prstDash/@val='dash'"><xsl:value-of select="'dash'"/></xsl:when><xsl:when test="a:prstDash/@val='lgDash'"><xsl:value-of select="'long-dash'"/></xsl:when><xsl:when test="a:prstDash/@val='dashDot'"><xsl:value-of select="'dash-dot'"/></xsl:when><xsl:when test="a:prstDash/@val='lgDashDot'"><xsl:value-of select="'dash-dot'"/></xsl:when><xsl:when test="a:prstDash/@val='lgDashDotDot'"><xsl:value-of select="'dash-dot-dot'"/></xsl:when><xsl:when test="a:prstDash/@val='sysDash'"><xsl:value-of select="'dash'"/></xsl:when><xsl:when test="a:prstDash/@val='sysDot'"><xsl:value-of select="'square-dot'"/></xsl:when><xsl:when test="a:prstDash/@val='sysDashDot'"><xsl:value-of select="'dash-dot'"/></xsl:when><xsl:when test="a:prstDash/@val='sysDashDotDot'"><xsl:value-of select="'dash-dot-dot'"/></xsl:when></xsl:choose></xsl:attribute>
						</图:线类型_8059>
					</xsl:if>
					<xsl:if test="not(a:prstDash/@val) and @cmpd">
						<图:线类型_8059>
							<xsl:attribute name="线型_805A"><xsl:choose><xsl:when test="@cmpd='dbl'"><xsl:value-of select="'double'"/></xsl:when><xsl:when test="@cmpd='sng'"><xsl:value-of select="'single'"/></xsl:when><!-- 20130402 update by xuzhenwei BUG_2727 集成OO-UOF 预定义图形大小不一致；填充，边框转换前后不一致；三维效果丢失；部分文本框对齐方式转换前后不一致。start --><!--上细下粗--><xsl:when test="@cmpd='thickThin'"><xsl:value-of select="'thin-thick'"/></xsl:when><!--上粗下细--><xsl:when test="@cmpd='thinThick'"><xsl:value-of select="'thick-thin'"/></xsl:when><!--三线--><xsl:when test="@cmpd='tri'"><xsl:value-of select="'thick-between-thin'"/></xsl:when></xsl:choose></xsl:attribute>
							<xsl:attribute name="虚实_805B"><xsl:value-of select="solid"/></xsl:attribute>
							<!-- end -->
						</图:线类型_8059>
					</xsl:if>
					<xsl:if test="a:prstDash/@val and @cmpd">
						<图:线类型_8059>
							<xsl:attribute name="线型_805A"><xsl:choose><xsl:when test="@cmpd='dbl'"><xsl:value-of select="'double'"/></xsl:when><xsl:when test="@cmpd='sng'"><xsl:value-of select="'single'"/></xsl:when><!-- 20130402 update by xuzhenwei BUG_2727 集成OO-UOF 预定义图形大小不一致；填充，边框转换前后不一致；三维效果丢失；部分文本框对齐方式转换前后不一致。start --><!--上细下粗--><xsl:when test="@cmpd='thickThin'"><xsl:value-of select="'thin-thick'"/></xsl:when><!--上粗下细--><xsl:when test="@cmpd='thinThick'"><xsl:value-of select="'thick-thin'"/></xsl:when><!--三线--><xsl:when test="@cmpd='tri'"><xsl:value-of select="'thick-between-thin'"/></xsl:when><!-- end --></xsl:choose></xsl:attribute>
							<xsl:attribute name="虚实_805B"><xsl:choose><xsl:when test="a:prstDash/@val='solid'"><xsl:value-of select="'solid'"/></xsl:when><xsl:when test="a:prstDash/@val='dot'"><xsl:value-of select="'square-dot'"/></xsl:when><xsl:when test="a:prstDash/@val='dash'"><xsl:value-of select="'dash'"/></xsl:when><xsl:when test="a:prstDash/@val='lgDash'"><xsl:value-of select="'long-dash'"/></xsl:when><xsl:when test="a:prstDash/@val='dashDot'"><xsl:value-of select="'dash-dot'"/></xsl:when><xsl:when test="a:prstDash/@val='lgDashDot'"><xsl:value-of select="'dash-dot'"/></xsl:when><xsl:when test="a:prstDash/@val='lgDashDotDot'"><xsl:value-of select="'dash-dot-dot'"/></xsl:when><xsl:when test="a:prstDash/@val='sysDash'"><xsl:value-of select="'dash'"/></xsl:when><xsl:when test="a:prstDash/@val='sysDot'"><xsl:value-of select="'square-dot'"/></xsl:when><xsl:when test="a:prstDash/@val='sysDashDot'"><xsl:value-of select="'dash-dot'"/></xsl:when><xsl:when test="a:prstDash/@val='sysDashDotDot'"><xsl:value-of select="'dash-dot-dot'"/></xsl:when></xsl:choose></xsl:attribute>
						</图:线类型_8059>
					</xsl:if>
		...
		<xsl:if test="not(a:ln) and following-sibling::xdr:style">
			<图:线_8057>
						...
						<xsl:choose>
							...
							<xsl:when test="following-sibling::xdr:style/a:lnRef/a:schemeClr">
								...
								<xsl:choose>
									...
									<xsl:when test="$id='accent1'">
										<!-- 2013-11-25 Qihy 默认图形轮廓颜色不正确 start-->
										<xsl:value-of select="'#333399'"/>
										<!-- <xsl:value-of select="'#4F81BD'"/> -->
										<!-- 2013-11-25 Qihy 默认图形轮廓颜色不正确 end-->
									</xsl:when>
									<xsl:when test="$id='accent2'">
										<xsl:value-of select="'#C0504D'"/>
									</xsl:when>
				...
				<xsl:if test="following-sibling::xdr:style">
					<图:线类型_8059>
						<xsl:attribute name="线型_805A"><xsl:variable name="lnstyleid"><xsl:value-of select="following-sibling::xdr:style/a:lnRef/@idx"/></xsl:variable><xsl:choose><!-- update by xuzhenwei BUG_2727: 集成OO-UOF 预定义图形大小不一致；填充，边框转换前后不一致；三维效果丢失；部分文本框对齐方式转换前后不一致。 start OO中的线型只有四种类型：sng、dbl、tri、thickThin和thinThick --><xsl:when test="//a:fmtScheme/a:lnStyleLst/a:ln[position()=$lnstyleid]/a:prstDash[not(@val='solid')]"><xsl:value-of select="//a:fmtScheme/a:lnStyleLst/a:ln[position()=$lnstyleid]/a:prstDash/@val"/></xsl:when><!-- end --><xsl:otherwise><xsl:value-of select="'single'"/></xsl:otherwise></xsl:choose></xsl:attribute>
					</图:线类型_8059>
				</xsl:if>
				<xsl:if test="following-sibling::xdr:style">
					<图:线粗细_805C>
	...
	<xsl:template match="a:bodyPr">
		<xsl:attribute name="是否为文本框_8046"><xsl:value-of select="'true'"/></xsl:attribute>
		<xsl:attribute name="是否自动换行_8047"><xsl:if test="./@wrap='none'"><xsl:value-of select="'false'"/></xsl:if><xsl:if test="./@wrap='square' or name(./@wrap)=''"><xsl:value-of select="'true'"/></xsl:if></xsl:attribute>
		<xsl:attribute name="是否大小适应文字_8048"><!--修改属性：是否大小适应文字 李杨2011-12-09--><!--<xsl:if test="concat(name(a:spAutofit),name(a:normAutofit))!=''">--><!--<xsl:if test ="$as='a:spAutofit'">--><xsl:choose><xsl:when test="./a:spAutoFit or ./a:normAutofit"><xsl:value-of select="'true'"/></xsl:when><xsl:when test="./a:spAutoFit and not(./a:normAutofit)"><xsl:value-of select="'true'"/></xsl:when><xsl:when test="not(./a:spAutoFit) and ./a:normAutofit"><xsl:value-of select="'true'"/></xsl:when><xsl:otherwise><xsl:value-of select="'false'"/></xsl:otherwise></xsl:choose><!--<xsl:if test ="./a:spAutoFit or ./a:normAutofit">
      ...
      <xsl:if test ="not(./a:spAutofit) and not(./a:normAutofit)">
		...
		<xsl:attribute name="是否文字随对象旋转_8049"><xsl:value-of select="'true'"/></xsl:attribute>
		<xsl:attribute name="是否文字绕排外部对象_8068"><xsl:value-of select="'true'"/></xsl:attribute>
		<!--2014-1-14, add, Qihy 修复BUG2986-文本框_预定义图形-内部边距，内部边距功能失效 start-->
		<图:边距_803D>
			<xsl:if test="@lIns">
				<xsl:attribute name="左_C608"><xsl:value-of select="./@lIns div 12700"/></xsl:attribute>
			</xsl:if>
			<xsl:if test="@tIns">
				<xsl:attribute name="上_C609"><xsl:value-of select="./@tIns div 12700"/></xsl:attribute>
			</xsl:if>
			<xsl:if test="@rIns">
				<xsl:attribute name="右_C60A"><xsl:value-of select="./@rIns div 12700"/></xsl:attribute>
			</xsl:if>
			<xsl:if test="@bIns">
				<xsl:attribute name="下_C60B"><xsl:value-of select="./@bIns div 12700"/></xsl:attribute>
			</xsl:if>
		</图:边距_803D>
		<!--end-->
		<图:对齐_803E>
			<xsl:attribute name="水平对齐_421D"><xsl:choose><!--2014-1-14, update, Qihy 修复BUG2985-文本框_预定义图形-文本框-内容-文本方向-垂直水平对齐中部居中转换错误 start--><xsl:when test="./@anchorCtr='1'"><xsl:value-of select="'center'"/></xsl:when><xsl:when test="./@vert='eaVert' or ./@vert='vert'"><xsl:value-of select="'right'"/></xsl:when><!--end--><xsl:otherwise><xsl:value-of select="'left'"/></xsl:otherwise></xsl:choose></xsl:attribute>
			<xsl:if test="./@anchor">
				<xsl:attribute name="文字对齐_421E"><xsl:choose><xsl:when test="./@anchor='b'"><xsl:value-of select="'bottom'"/></xsl:when><xsl:when test="./@anchor='ctr' or ./@anchor='dist'"><!--<xsl:value-of select="'middle'"/>--><xsl:value-of select="'center'"/></xsl:when><xsl:when test="./@anchor='t'"><xsl:value-of select="'top'"/></xsl:when><xsl:when test="./@anchor='just'"><!--<xsl:value-of select="'justify'"/>--><xsl:value-of select="base"/></xsl:when></xsl:choose></xsl:attribute>
			</xsl:if>
		</图:对齐_803E>
		<xsl:if test="./@vert">
				...
				<字:字体_4128>
					...
					<xsl:if test="ancestor::xdr:txBody">
						...
						<xsl:if test="not(a:latin or a:ea  or a:cs)">
							<xsl:attribute name="西文字体引用_4129"><xsl:value-of select="'font_00002'"/></xsl:attribute>
							<xsl:attribute name="中文字体引用_412A"><xsl:value-of select="'font_00001'"/></xsl:attribute>
						</xsl:if>
						...
						<xsl:attribute name="中文字体引用_412A">
							...
							<xsl:if test="a:latin">
								<xsl:attribute name="西文字体引用_4129"><xsl:value-of select="concat($zitiyy,'latin')"/></xsl:attribute>
							</xsl:if>
							<xsl:if test="a:ea  or a:cs">
								<xsl:attribute name="中文字体引用_412A"><xsl:value-of select="concat($zitiyy,'ea')"/></xsl:attribute>
							</xsl:if>
						</xsl:if>
					</xsl:if>
					<xsl:if test="@lang and @lang ='en-US'">
						<xsl:attribute name="是否西文绘制_412C"><xsl:value-of select="'true'"/></xsl:attribute>
					</xsl:if>
					<xsl:if test="@lang and @lang = 'zh-CN'">
						<xsl:attribute name="是否西文绘制_412C"><xsl:value-of select="'false'"/></xsl:attribute>
					</xsl:if>
					<!--2014-3-29, end-->
					<!--end-->
					<xsl:if test="@sz">
						<xsl:attribute name="字号_412D"><xsl:value-of select="@sz div 100"/></xsl:attribute>
					</xsl:if>
					<!--yx,add,2010.4.28-->
					<xsl:attribute name="颜色_412F"><xsl:choose><!--2014-3-10, update by Qihy, 修复BUG3075 集成测试ooxml-uof（20102013），部分艺术字转换后丢失以及字体颜色转换不正确（UOF不支持渐变，渐变效果丢失） start--><xsl:when test="a:solidFill/a:srgbClr/@val = 'FFFFFF' or a:ln/a:solidFill/a:srgbClr/@val = 'FFFFFF'"><xsl:value-of select="'#000000'"/></xsl:when><xsl:when test="a:solidFill/a:srgbClr"><xsl:value-of select="concat('#',a:solidFill/a:srgbClr/@val)"/></xsl:when><xsl:when test="a:ln/a:solidFill/a:srgbClr and a:ln/a:solidFill/a:srgbClr"><xsl:value-of select="concat('#',a:ln/a:solidFill/a:srgbClr/@val)"/></xsl:when><xsl:when test="a:solidFill/a:schemeClr or a:ln/a:solidFill/a:schemeClr or a:gradFill/a:gsLst/a:gs/a:schemeClr or ./ancestor::xdr:txBody[1]/preceding-sibling::xdr:style/a:fontRef/a:schemeClr"><xsl:variable name="id"><xsl:choose><xsl:when test="a:solidFill/a:schemeClr"><xsl:value-of select="a:solidFill/a:schemeClr/@val"/></xsl:when><xsl:when test="a:ln/a:solidFill/a:schemeClr"><xsl:value-of select="a:ln/a:solidFill/a:schemeClr/@val"/></xsl:when><xsl:when test="a:gradFill/a:gsLst/a:gs/a:schemeClr"><xsl:value-of select="a:gradFill/a:gsLst/a:gs/a:schemeClr/@val"/></xsl:when><xsl:when test="./ancestor::xdr:txBody[1]/preceding-sibling::xdr:style/a:fontRef/a:schemeClr"><xsl:value-of select="./ancestor::xdr:txBody[1]/preceding-sibling::xdr:style/a:fontRef/a:schemeClr/@val"/></xsl:when></xsl:choose></xsl:variable><xsl:choose><xsl:when test="$id='tx1'"><xsl:value-of select="'#000000'"/></xsl:when><xsl:when test="$id='dk1'"><xsl:value-of select="'#000000'"/></xsl:when><xsl:when test="$id='lt1'"><xsl:value-of select="'#FFFFFF'"/></xsl:when><xsl:when test="$id='dk2'"><xsl:value-of select="'#1F497D'"/></xsl:when><xsl:when test="$id='lt2'"><xsl:value-of select="'#EEECE1'"/></xsl:when><xsl:when test="$id='accent1'"><xsl:value-of select="'#4F81BD'"/></xsl:when><xsl:when test="$id='accent2'"><xsl:value-of select="'#C0504D'"/></xsl:when><xsl:when test="$id='accent3'"><xsl:value-of select="'#9BBB59'"/></xsl:when><xsl:when test="$id='accent4'"><xsl:value-of select="'#C0C0C0'"/></xsl:when><xsl:when test="$id='accent5'"><xsl:value-of select="'#4BACC6'"/></xsl:when><xsl:when test="$id='accent6'"><xsl:value-of select="'#F79646'"/></xsl:when><xsl:when test="$id='hlink'"><xsl:value-of select="'#0000FF'"/></xsl:when><xsl:when test="$id='folHlink'"><xsl:value-of select="'#800080'"/></xsl:when><!--2014-5-5, add by Qihy, bug3302,文本框中字体颜色不正确， start--><xsl:when test="$id='bg1'"><xsl:value-of select="'#FFFFFF'"/></xsl:when><!--2014-5-5 end--><xsl:otherwise><xsl:value-of select="'auto'"/></xsl:otherwise></xsl:choose></xsl:when><xsl:when test="not(a:solidFill) and a:gradFill/a:gsLst/a:gs[position()=2]/a:srgbClr"><xsl:value-of select="concat('#',a:gradFill/a:gsLst/a:gs[position()=2]/a:srgbClr/@val)"/></xsl:when><xsl:when test="./ancestor::xdr:txBody[1]/preceding-sibling::xdr:style/a:fontRef/a:srgbClr"><xsl:value-of select="concat('#',preceding::xdr:style/a:fontRef/a:srgbClr/@val)"/></xsl:when><!--2014-3-10 end--></xsl:choose></xsl:attribute>
				</字:字体_4128>
			</xsl:if>
			<xsl:if test="@b">
				<字:是否粗体_4130>
			...
			<字:下划线_4136>
				...
				<xsl:if test="@u">
					<xsl:choose>
						...
						<xsl:otherwise>
							<xsl:attribute name="线型_4137"><xsl:value-of select="@u"/></xsl:attribute>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:if>
				...
				<xsl:if test="a:uFill/a:solidFill/a:srgbClr">
					<xsl:attribute name="颜色_412F"><xsl:value-of select="concat('#', a:uFill/a:solidFill/a:srgbClr/@val)"/></xsl:attribute>
				</xsl:if>
			</字:下划线_4136>
			<xsl:if test="@baseline">
				<!--2014-3-23 update by Qihy, strict ooxml中baseline取值以百分数（%）表示，将全文中的baseline取值修改(下文全部修改) start-->
				<字:上下标类型_4143>
					<xsl:if test="@baseline = '0%'">
						<xsl:value-of select="'none'"/>
					</xsl:if>
					<xsl:if test="@baseline='30%'">
						<xsl:value-of select="'sup'"/>
					</xsl:if>
					<xsl:if test="@baseline='-25%'">
						<xsl:value-of select="'sub'"/>
					</xsl:if>
				</字:上下标类型_4143>
				<!--2014-3-23 end-->
	...
	<xsl:template name="pPr-body">
		<xsl:if test="@algn|@fontAlgn">
			<字:对齐_417D>
				<xsl:if test="@algn">
					<xsl:attribute name="水平对齐_421D"><xsl:choose><xsl:when test="@algn='l'">left</xsl:when><xsl:when test="@algn='r'">right</xsl:when><xsl:when test="@algn='ctr'">center</xsl:when><xsl:when test="@algn='just' or @algn='justLow'">justified</xsl:when><xsl:when test="@algn='dist'or @algn='thaiDist'">distributed</xsl:when></xsl:choose></xsl:attribute>
				</xsl:if>
				<xsl:if test="@fontAlgn">
					<xsl:attribute name="文字对齐_421E"><xsl:choose><xsl:when test="@fontAlgn='auto'">auto</xsl:when><xsl:when test="@fontAlgn='t'">top</xsl:when><xsl:when test="@fontAlgn='ctr'">center</xsl:when><xsl:when test="@fontAlgn='base'">base</xsl:when><xsl:when test="@fontAlgn='b'">bottom</xsl:when></xsl:choose></xsl:attribute>
				</xsl:if>
			</字:对齐_417D>
		</xsl:if>
		<!--缩进-->
			...
			<字:缩进_411D>
				<xsl:choose>
					...
					<xsl:when test="@marL">
						<字:左_410E>
							<字:绝对_4107>
								<xsl:attribute name="值_410F"><xsl:value-of select="@marL div 12700"/></xsl:attribute>
							</字:绝对_4107>
						</字:左_410E>
					</xsl:when>
					<xsl:when test="@marR">
						<字:右_4110>
							<字:绝对_4107>
								<xsl:attribute name="值_410F"><xsl:value-of select="@marR div 12700"/></xsl:attribute>
							</字:绝对_4107>
						</字:右_4110>
					</xsl:when>
					<xsl:when test="@indent">
						<字:首行_4111>
							<字:绝对_4107>
								<xsl:attribute name="值_410F"><xsl:value-of select="@indent div 12700"/></xsl:attribute>
							</字:绝对_4107>
						</字:首行_4111>
					</xsl:when>
				</xsl:choose>
		...
		<!--行距-->
		<!--2014-3-19, update by Qihy, 修复行距取值为NaN的问题，修复智能标记需要恢复打开的问题 start-->
		<!--<xsl:if test="a:lnSpc">
      <字:行距_417E>
      ...
      <字:段间距_4180>
		...
		<xsl:if test="a:lnSpc">
			<字:行距_417E>
				<xsl:if test="a:lnSpc/a:spcPct">
					<xsl:attribute name="类型_417F">multi-lines</xsl:attribute>
					<xsl:attribute name="值_4108"><xsl:value-of select="number(substring-before(a:lnSpc/a:spcPct/@val, '%')) div 100"/></xsl:attribute>
				</xsl:if>
				<xsl:if test="a:lnSpc/a:spcPts">
					<xsl:attribute name="类型_417F">fixed</xsl:attribute>
					<xsl:attribute name="值_4108"><xsl:value-of select="a:lnSpc/a:spcPts/@val div 100"/></xsl:attribute>
				</xsl:if>
				<!--最小值、行间距 无对应-->
			</字:行距_417E>
		</xsl:if>
			...
			<字:段间距_4180>
				<xsl:if test="a:spcBef">
					<字:段前距_4181>
						<xsl:if test="a:spcBef/a:spcPct">
							<字:相对值_4184>
								<xsl:value-of select="number(substring-before(a:spcBef/a:spcPct/@val, '%')) div 100"/>
							</字:相对值_4184>
						</xsl:if>
						<xsl:if test="a:spcBef/a:spcPts">
							<字:绝对值_4183>
				...
				<xsl:if test="a:spcAft">
					<字:段后距_4185>
						...
						<xsl:if test="a:spcAft/a:spcPct">
							<!-- end -->
							<字:相对值_4184>
								<xsl:value-of select="number(substring-before(a:spcAft/a:spcPct/@val, '%')) div 100"/>
							</字:相对值_4184>
						</xsl:if>
						<xsl:if test="a:spcAft/a:spcPts">
							<字:绝对值_4183>
		...
		<xsl:if test="a:buChar|a:buAutoNum|a:buBlip">
			<字:自动编号信息_4186>
				<xsl:attribute name="编号引用_4187"><xsl:if test="@id"><xsl:value-of select="concat('bn_',@id)"/></xsl:if><!--p/pPr/bu...的情况 --><xsl:if test="not(@id)"><xsl:value-of select="concat('bn_',translate(../@id,':',''))"/></xsl:if></xsl:attribute>
	...
	<xsl:template match="xdr:sp|dsp:sp">
		...
		<xsl:param name="twoCellAnchorPosi"/>
		<!-- add by xuzhenwei BUG_2910 第二轮回归 oo-uof 功能 文本框字体不正确，该变量用来控制与style.xsl中<式样:字体声明_990D>计算字体名称的方式保持一致  -->
		<xsl:param name="posi"/>
		<!-- end -->
		<图:图形_8062>
			...
			<xsl:attribute name="标识符_804B"><xsl:value-of select="concat($s_name,'_OBJ0000',$ppid)"/></xsl:attribute>
			<xsl:attribute name="层次_8063"><xsl:choose><xsl:when test="name(.)='dsp:sp'"><xsl:value-of select="'1'"/></xsl:when><xsl:otherwise><xsl:number count="xdr:twoCellAnchor" level="single"/></xsl:otherwise></xsl:choose></xsl:attribute>
			<图:预定义图形_8018>
			...
			<!--2013-12-16 Qihy 添加对象-控制点的转换实现 start -->
			<xsl:choose>
				...
				<xsl:when test="(./xdr:spPr/a:prstGeom/@prst='wedgeEllipseCallout' or ./xdr:spPr/a:prstGeom/@prst='wedgeRoundRectCallout' or ./xdr:spPr/a:prstGeom/@prst='wedgeRectCallout' or ./xdr:spPr/a:prstGeom/@prst='cloudCallout') and ./*/a:prstGeom/a:avLst/a:gd">
					...
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="($x * 21600) div 100000 + 10800"/></xsl:attribute>
						<xsl:attribute name="y_C607"><xsl:value-of select="($y * 21600) div 100000 + 10800"/></xsl:attribute>
					</图:控制点_8039>
				</xsl:when>
				...
				<xsl:when test="(./xdr:spPr/a:prstGeom/@prst='borderCallout1' or ./xdr:spPr/a:prstGeom/@prst='accentCallout1' or ./xdr:spPr/a:prstGeom/@prst='callout1' or ./xdr:spPr/a:prstGeom/@prst='accentBorderCallout1') and ./*/a:prstGeom/a:avLst/a:gd">
					...
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="($x2 * 21600) div 100000"/></xsl:attribute>
						<xsl:attribute name="y_C607"><xsl:value-of select="($y2 * 21600) div 100000"/></xsl:attribute>
					</图:控制点_8039>
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="($x1 * 21600) div 100000"/></xsl:attribute>
						<xsl:attribute name="y_C607"><xsl:value-of select="($y1 * 21600) div 100000"/></xsl:attribute>
					</图:控制点_8039>
				</xsl:when>
				...
				<xsl:when test="(./xdr:spPr/a:prstGeom/@prst='borderCallout2' or ./xdr:spPr/a:prstGeom/@prst='accentCallout2' or ./xdr:spPr/a:prstGeom/@prst='callout2' or ./xdr:spPr/a:prstGeom/@prst='accentBorderCallout2') and ./*/a:prstGeom/a:avLst/a:gd">
					...
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="($x3 * 21600) div 100000"/></xsl:attribute>
						<xsl:attribute name="y_C607"><xsl:value-of select="($y3 * 21600) div 100000"/></xsl:attribute>
					</图:控制点_8039>
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="($x2 * 21600) div 100000"/></xsl:attribute>
						<xsl:attribute name="y_C607"><xsl:value-of select="($y2 * 21600) div 100000"/></xsl:attribute>
				...
				<xsl:when test="(./xdr:spPr/a:prstGeom/@prst='borderCallout3' or ./xdr:spPr/a:prstGeom/@prst='accentCallout3' or ./xdr:spPr/a:prstGeom/@prst='callout3' or ./xdr:spPr/a:prstGeom/@prst='accentBorderCallout3') and ./*/a:prstGeom/a:avLst/a:gd">
					...
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="($x4 * 21600) div 100000"/></xsl:attribute>
						<xsl:attribute name="y_C607"><xsl:value-of select="($y4 * 21600) div 100000"/></xsl:attribute>
					</图:控制点_8039>
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="($x3 * 21600) div 100000"/></xsl:attribute>
						<xsl:attribute name="y_C607"><xsl:value-of select="($y3 * 21600) div 100000"/></xsl:attribute>
				...
				<xsl:when test="(./xdr:spPr/a:prstGeom/@prst='star4' or ./xdr:spPr/a:prstGeom/@prst='star8' or ./xdr:spPr/a:prstGeom/@prst='star16' or ./xdr:spPr/a:prstGeom/@prst='star24' or ./xdr:spPr/a:prstGeom/@prst='star32' or ./xdr:spPr/a:prstGeom/@prst='star6' or ./xdr:spPr/a:prstGeom/@prst='star7' or ./xdr:spPr/a:prstGeom/@prst='star10' or ./xdr:spPr/a:prstGeom/@prst='star12') and ./*/a:prstGeom/a:avLst/a:gd">
					...
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="10800 - ($x * 21600) div 100000"/></xsl:attribute>
					</图:控制点_8039>
				</xsl:when>
				<!--上凸带形-->
				<xsl:when test="(./xdr:spPr/a:prstGeom/@prst='ribbon2') and ./*/a:prstGeom/a:avLst/a:gd">
					...
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="10800 - ($x * 21600) div 200000"/></xsl:attribute>
						<xsl:attribute name="y_C607"><xsl:value-of select="21600 - ($y * 21600) div 100000"/></xsl:attribute>
					</图:控制点_8039>
				</xsl:when>
				<!--前凸带形-->
				<xsl:when test="(./xdr:spPr/a:prstGeom/@prst='ribbon') and ./*/a:prstGeom/a:avLst/a:gd">
					...
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="10800 - ($x * 21600) div 200000"/></xsl:attribute>
						<xsl:attribute name="y_C607"><xsl:value-of select="($y * 21600) div 100000"/></xsl:attribute>
					</图:控制点_8039>
				</xsl:when>
				<!--上凸弯带形-->
				<xsl:when test="(./xdr:spPr/a:prstGeom/@prst='ellipseRibbon2') and ./*/a:prstGeom/a:avLst/a:gd">
					...
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="10800 - ($x1 * 21600) div 200000"/></xsl:attribute>
						<xsl:attribute name="y_C607"><xsl:value-of select="21600 - ($y1 * 21600) div 100000"/></xsl:attribute>
					</图:控制点_8039>
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="($x2 * 21600) div 100000 + 675"/></xsl:attribute>
					</图:控制点_8039>
				</xsl:when>
				<!--前凸弯带形-->
				<xsl:when test="(./xdr:spPr/a:prstGeom/@prst='ellipseRibbon') and ./*/a:prstGeom/a:avLst/a:gd">
					...
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="21600 - ($x2 * 21600) div 100000 - 675"/></xsl:attribute>
					</图:控制点_8039>
				</xsl:when>
				<!--竖卷形，横卷形-->
				<xsl:when test="(./xdr:spPr/a:prstGeom/@prst='verticalScroll' or ./xdr:spPr/a:prstGeom/@prst='horizontalScroll') and ./*/a:prstGeom/a:avLst/a:gd">
					...
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="($x * 21600) div 100000"/></xsl:attribute>
					</图:控制点_8039>
				</xsl:when>
				<!--波形，双波形-->
				<xsl:when test="(./xdr:spPr/a:prstGeom/@prst='wave' or ./xdr:spPr/a:prstGeom/@prst='doubleWave') and ./*/a:prstGeom/a:avLst/a:gd">
					...
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="($x * 21600) div 100000"/></xsl:attribute>
						<xsl:attribute name="y_C607"><xsl:value-of select="10800 + ($y * 21600) div 100000"/></xsl:attribute>
					</图:控制点_8039>
				</xsl:when>
				<!--右箭头，虚尾箭头，燕尾形箭头-->
				<xsl:when test="(./xdr:spPr/a:prstGeom/@prst='rightArrow' or ./xdr:spPr/a:prstGeom/@prst='stripedRightArrow' or ./xdr:spPr/a:prstGeom/@prst='notchedRightArrow') and ./*/a:prstGeom/a:avLst/a:gd">
					...
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="21600 - ($x * 21600 * $height) div $width div 100000"/></xsl:attribute>
						<xsl:attribute name="y_C607"><xsl:value-of select="10800 - ($y * 10800) div 100000"/></xsl:attribute>
					</图:控制点_8039>
				</xsl:when>
				<!--左箭头，左右箭头-->
				<xsl:when test="(./xdr:spPr/a:prstGeom/@prst='leftArrow' or ./xdr:spPr/a:prstGeom/@prst='leftRightArrow') and ./*/a:prstGeom/a:avLst/a:gd">
					...
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="($x * 21600 * $height) div $width div 100000"/></xsl:attribute>
						<xsl:attribute name="y_C607"><xsl:value-of select="10800 - ($y * 10800) div 100000"/></xsl:attribute>
					</图:控制点_8039>
				</xsl:when>
				<!--上箭头-->
				<xsl:when test="(./xdr:spPr/a:prstGeom/@prst='upArrow') and ./*/a:prstGeom/a:avLst/a:gd">
					...
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="($y * 21600 * $width) div $height div 100000"/></xsl:attribute>
						<xsl:attribute name="y_C607"><xsl:value-of select="10800 - ($x * 10800) div 100000"/></xsl:attribute>
					</图:控制点_8039>
				</xsl:when>
				<!--上下箭头-->
				<xsl:when test="(./xdr:spPr/a:prstGeom/@prst='upDownArrow') and ./*/a:prstGeom/a:avLst/a:gd">
					...
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="10800 - ($x * 10800) div 100000"/></xsl:attribute>
						<xsl:attribute name="y_C607"><xsl:value-of select="($y * 21600 * $width) div $height div 100000 "/></xsl:attribute>
					</图:控制点_8039>
				</xsl:when>
				<!--下箭头-->
				<xsl:when test="(./xdr:spPr/a:prstGeom/@prst='downArrow') and ./*/a:prstGeom/a:avLst/a:gd">
					...
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="21600 - ($y * 21600 * $width) div $height div 100000"/></xsl:attribute>
						<xsl:attribute name="y_C607"><xsl:value-of select="10800 - ($x * 10800) div 100000"/></xsl:attribute>
					</图:控制点_8039>
				</xsl:when>
				<!--十字箭头，丁字箭头-->
				<xsl:when test="(./xdr:spPr/a:prstGeom/@prst='quadArrow' or ./xdr:spPr/a:prstGeom/@prst='leftRightUpArrow') and ./*/a:prstGeom/a:avLst/a:gd">
					...
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="10800 - ($x1 * 10800) div 50000"/></xsl:attribute>
						<xsl:attribute name="y_C607"><xsl:value-of select="10800 - ($y1 * 10800) div 100000"/></xsl:attribute>
					</图:控制点_8039>
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="$x2 * 10800 div 50000"/></xsl:attribute>
					</图:控制点_8039>
				</xsl:when>
				<!--圆角右箭头-->
				<xsl:when test="(./xdr:spPr/a:prstGeom/@prst='bentArrow') and ./*/a:prstGeom/a:avLst/a:gd">
					...
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="21600 - ($x * $height * 21600) div $width div 100000"/></xsl:attribute>
						<xsl:attribute name="y_C607"><xsl:value-of select="6079 - ($y * $height * 10800) div $width div 100000"/></xsl:attribute>
					</图:控制点_8039>
				</xsl:when>
				<!--直角双向箭头，直角上箭头-->
				<xsl:when test="(./xdr:spPr/a:prstGeom/@prst='leftUpArrow' or ./xdr:spPr/a:prstGeom/@prst='bentUpArrow') and ./*/a:prstGeom/a:avLst/a:gd">
					...
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="21600 - ($x1 * $height * 21600) div $width div 50000"/></xsl:attribute>
						<xsl:attribute name="y_C607"><xsl:value-of select="21600 - ($x1 * $height * 21600) div $width div 100000 + ($y1 * $height * 21600) div $width div 200000"/></xsl:attribute>
					</图:控制点_8039>
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="$x2 * 10800 div 50000"/></xsl:attribute>
					</图:控制点_8039>
				...
				<xsl:when test="(./xdr:spPr/a:prstGeom/@prst='curvedRightArrow') and ./*/a:prstGeom/a:avLst/a:gd">
					...
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><!--<xsl:value-of select="21600 - $x2 * $x2 * 21600 div 100000 div 100000 div 2 - $x1 * $width *  21600 div $height div 100000"/>--><xsl:value-of select="21600 - $x1 * $width *  21600 div $height div 100000 + 250"/></xsl:attribute>
						<xsl:attribute name="y_C607"><!--<xsl:value-of select="21600 - $x2 * $x2 * 21600 div 100000 div 100000 div 2 - ($x1 - $y1) * $width * 21600 div $height div 200000"/>--><xsl:value-of select="21600 - ($x1 - $y1) * $width * 21600 div $height div 200000 + 250"/></xsl:attribute>
					</图:控制点_8039>
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="21600 - $x2 * 21600 div 100000"/></xsl:attribute>
					</图:控制点_8039>
				</xsl:when>
				<!--右弧形箭头，显示效果有差异-->
				...
				<xsl:when test="(./xdr:spPr/a:prstGeom/@prst='curvedUpArrow') and ./*/a:prstGeom/a:avLst/a:gd">
					...
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><!--<xsl:value-of select="21600 - $x2 * $x2 * 21600 div 100000 div 100000 div 2 - $x1 * $height *  21600 div $width div 100000"/>--><xsl:value-of select="21600 - $x1 * $height *  21600 div $width div 100000 + 10"/></xsl:attribute>
						<xsl:attribute name="y_C607"><!--<xsl:value-of select="21600 - $x2 * $x2 * 21600 div 100000 div 100000 div 2 - ($x1 - $y1) * $height * 21600 div $width div 200000"/>--><xsl:value-of select="21600 - ($x1 - $y1) * $height * 21600 div $width div 200000 + 10"/></xsl:attribute>
					</图:控制点_8039>
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="$x2 * 21600 div 100000"/></xsl:attribute>
					</图:控制点_8039>
				...
				<xsl:when test="(./xdr:spPr/a:prstGeom/@prst='rightArrowCallout') and ./*/a:prstGeom/a:avLst/a:gd">
					...
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="($x1 * 21600) div 100000"/></xsl:attribute>
						<xsl:attribute name="y_C607"><xsl:value-of select="10800 - ($y1 * 21600) div 100000"/></xsl:attribute>
					</图:控制点_8039>
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="21600 - ($x2 * 21600 * $height) div $width div 100000"/></xsl:attribute>
						<xsl:attribute name="y_C607"><xsl:value-of select="10800 - ($y2 * 21600) div 200000"/></xsl:attribute>
					</图:控制点_8039>
				</xsl:when>
				<!--左箭头标注-->
				<xsl:when test="(./xdr:spPr/a:prstGeom/@prst='leftArrowCallout') and ./*/a:prstGeom/a:avLst/a:gd">
					...
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="21600 - ($x1 * 21600) div 100000"/></xsl:attribute>
						<xsl:attribute name="y_C607"><xsl:value-of select="10800 - ($y1 * 21600) div 100000"/></xsl:attribute>
					</图:控制点_8039>
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="($x2 * 21600 * $height) div $width div 100000"/></xsl:attribute>
						<xsl:attribute name="y_C607"><xsl:value-of select="10800 - ($y2 * 21600) div 200000"/></xsl:attribute>
					</图:控制点_8039>
				</xsl:when>
				<!--上箭头标注-->
				<xsl:when test="(./xdr:spPr/a:prstGeom/@prst='upArrowCallout') and ./*/a:prstGeom/a:avLst/a:gd">
					...
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="($x2 * 21600 * $width) div $height div 100000"/></xsl:attribute>
						<xsl:attribute name="y_C607"><xsl:value-of select="10800 - ($y2 * 21600) div 200000"/></xsl:attribute>
					</图:控制点_8039>
				</xsl:when>
				<!--下箭头标注-->
				<xsl:when test="(./xdr:spPr/a:prstGeom/@prst='downArrowCallout') and ./*/a:prstGeom/a:avLst/a:gd">
					...
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="21600 - ($x2 * 21600 * $width) div $height div 100000"/></xsl:attribute>
						<xsl:attribute name="y_C607"><xsl:value-of select="10800 - ($y2 * 21600) div 200000"/></xsl:attribute>
					</图:控制点_8039>
				</xsl:when>
				<!--左右箭头标注-->
				...
				<xsl:when test="(./xdr:spPr/a:prstGeom/@prst='quadArrowCallout') and ./*/a:prstGeom/a:avLst/a:gd">
					...
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="10800 - ($x1 * 21600) div 200000"/></xsl:attribute>
						<xsl:attribute name="y_C607"><xsl:value-of select="10800 - ($y1 * 21600 * $height) div $width div 100000"/></xsl:attribute>
					</图:控制点_8039>
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="$x2 * 21600 div 100000"/></xsl:attribute>
						<xsl:attribute name="y_C607"><xsl:value-of select="10800 - ($y2 * 21600 * $height) div $width div 200000"/></xsl:attribute>
					</图:控制点_8039>
				</xsl:when>
				...
				<xsl:when test="(./xdr:spPr/a:prstGeom/@prst='can') and ./*/a:prstGeom/a:avLst/a:gd">
					...
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="$y * 21600 div 200000"/></xsl:attribute>
					</图:控制点_8039>
				</xsl:when>
				<!--折角形-->
				<xsl:when test="(./xdr:spPr/a:prstGeom/@prst='foldedCorner') and ./*/a:prstGeom/a:avLst/a:gd">
					...
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="21600 - $x * 21600 div 100000"/></xsl:attribute>
					</图:控制点_8039>
				</xsl:when>
				<!--笑脸-->
				<xsl:when test="(./xdr:spPr/a:prstGeom/@prst='smileyFace') and ./*/a:prstGeom/a:avLst/a:gd">
					...
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="16200 + $y * 21600 div 100000"/></xsl:attribute>
					</图:控制点_8039>
				</xsl:when>
				<!--左中括号，右中括号-->
				...
				<xsl:when test="(./xdr:spPr/a:prstGeom/@prst='leftBrace' or ./xdr:spPr/a:prstGeom/@prst='rightBrace') and ./*/a:prstGeom/a:avLst/a:gd">
					...
					<图:控制点_8039>
						<xsl:attribute name="x_C606"><xsl:value-of select="$x * 21600 * $width div $height div 100000"/></xsl:attribute>
						<xsl:attribute name="y_C607"><xsl:value-of select="$y * 21600 div 100000"/></xsl:attribute>
					</图:控制点_8039>
				</xsl:when>
			...
			<!--2013-12-16 Qihy 添加对象-控制点的转换实现 end -->
			...
			<xsl:if test="ancestor::dsp:drawing">
				<图:组合位置_803B>
					<xsl:call-template name="groupLocationSmart">
						<xsl:with-param name="twoCellAnchorPosition" select="$twoCellAnchorPosi"/>
					</xsl:call-template>
				</图:组合位置_803B>
			</xsl:if>
			<!-- end -->
	...
	<xsl:template match="xdr:grpSp">
		...
		<图:图形_8062>
			...
			<xsl:attribute name="标识符_804B"><xsl:value-of select="concat($s_name,'_OBJ0000',$agrid)"/></xsl:attribute>
			<xsl:variable name="agr">
				<xsl:number count="xdr:grpSp" level="single"/>
			</xsl:variable>
			<xsl:attribute name="层次_8063"><xsl:value-of select="$agr - 1"/></xsl:attribute>
			<xsl:attribute name="组合列表_8064"><!-- update by xuzhenwei 调整一下输出内容组合列表的顺序，以便于uof到OO方向进行处理 start --><xsl:for-each select="xdr:pic[(preceding::xdr:grpSp)]"><xsl:variable name="id"><xsl:value-of select="xdr:nvPicPr/xdr:cNvPr/@id"/></xsl:variable><xsl:value-of select="concat($s_name,'_OBJ0000',$id,' ')"/></xsl:for-each><xsl:for-each select="xdr:pic[not(preceding::xdr:grpSp)]"><xsl:variable name="id"><xsl:value-of select="xdr:nvPicPr/xdr:cNvPr/@id"/></xsl:variable><xsl:if test="position()!=last()"><xsl:value-of select="concat($s_name,'_OBJ0000',$id,' ')"/></xsl:if><xsl:if test="position()=last()"><xsl:value-of select="concat($s_name,'_OBJ0000',$id,' ')"/></xsl:if></xsl:for-each><xsl:for-each select="xdr:grpSp|xdr:sp|xdr:cxnSp"><xsl:variable name="id"><xsl:choose><xsl:when test="xdr:nvSpPr"><xsl:value-of select="xdr:nvSpPr/xdr:cNvPr/@id"/></xsl:when><xsl:when test="xdr:nvCxnSpPr"><xsl:value-of select="xdr:nvCxnSpPr/xdr:cNvPr/@id"/></xsl:when><xsl:when test="xdr:nvGrpSpPr"><xsl:value-of select="xdr:nvGrpSpPr/xdr:cNvPr/@id"/></xsl:when></xsl:choose></xsl:variable><xsl:if test="position()!=last()"><xsl:value-of select="concat($s_name,'_OBJ0000',$id,' ')"/></xsl:if><xsl:if test="position()=last()"><xsl:value-of select="concat($s_name,'_OBJ0000',$id,' ')"/></xsl:if></xsl:for-each><!-- end --><xsl:if test="xdr:grpSp/xdr:pic"><xsl:for-each select="xdr:grpSp/xdr:pic"><xsl:variable name="id"><xsl:value-of select="xdr:nvPicPr/xdr:cNvPr/@id"/></xsl:variable><xsl:if test="position()!=last()"><xsl:value-of select="concat($s_name,'_OBJ0000',$id,' ')"/></xsl:if><xsl:if test="position()=last()"><xsl:value-of select="concat($s_name,'_OBJ0000',$id,' ')"/></xsl:if></xsl:for-each></xsl:if></xsl:attribute>
			<图:预定义图形_8018>
	...
	<xsl:template name="groupLocation">
		<!--<图:组合位置_803B>-->
		<xsl:attribute name="x_C606"><xsl:value-of select=".//a:xfrm/a:off/@x div 12700"/></xsl:attribute>
		<xsl:attribute name="y_C607"><xsl:value-of select=".//a:xfrm/a:off/@y div 12700"/></xsl:attribute>
		<!--</图:组合位置_803B>-->
	</xsl:template>
	...
	<xsl:template name="groupLocationSmart">
		...
		<xsl:attribute name="x_C606"><xsl:value-of select=".//a:xfrm/a:off/@x div 12700 + $xDistance"/></xsl:attribute>
		<xsl:attribute name="y_C607"><xsl:value-of select=".//a:xfrm/a:off/@y div 12700 + $yDistance"/></xsl:attribute>
		<!--</图:组合位置_803B>-->
	</xsl:template>
	<xsl:template name="dspdrawing">
		...
		<!-- 2014-04-24, update by qihouying, bug3156 集成测试ooxml->uof（2013），智能标记转换后丢失 start -->
      ...
      <xsl:apply-templates select=".">
				...
				<xsl:with-param name="twoCellAnchorPosi" select="$twoCellAnchorPos"/>
			</xsl:apply-templates>
		</xsl:for-each>
		<图:图形_8062>
			<xsl:variable name="drawingname" select="substring-before($drawingId,'.xml')"/>
			<xsl:attribute name="标识符_804B"><xsl:value-of select="concat($s_name,'_OBJ0000',$drawingname)"/></xsl:attribute>
			<xsl:attribute name="层次_8063"><xsl:value-of select="'0'"/></xsl:attribute>
			<xsl:attribute name="组合列表_8064"><xsl:for-each select="parent::xdr:wsDr/dsp:drawing[@id=$drawingId]//dsp:sp"><xsl:variable name="s_number" select="position()"/><xsl:value-of select="concat($s_name,'_OBJ0000',$drawingname,$s_number,$s_number,' ')"/></xsl:for-each></xsl:attribute>
			<图:预定义图形_8018>
				<图:类别_8019>11</图:类别_8019>
				<图:名称_801A>Rectangle</图:名称_801A>
				<图:生成软件_801B>Microsoft Office</图:生成软件_801B>
				<图:属性_801D>
					...
					<图:大小_8060>
						...
						<xsl:attribute name="长_C604"><xsl:choose><xsl:when test="contains($hit,'-')"><xsl:value-of select="translate($hit,'-','')"/></xsl:when><xsl:otherwise><xsl:value-of select="$hit"/></xsl:otherwise></xsl:choose></xsl:attribute>
						<xsl:attribute name="宽_C605"><xsl:choose><xsl:when test="contains($wit,'-')"><xsl:value-of select="translate($wit,'-','')"/></xsl:when><xsl:otherwise><xsl:value-of select="$wit"/></xsl:otherwise></xsl:choose></xsl:attribute>
					</图:大小_8060>
				</图:属性_801D>
			</图:预定义图形_8018>
		</图:图形_8062>
	...
	<xsl:template match="xdr:grpSp" mode="single">
		...
		<图:图形_8062>
			...
			<xsl:attribute name="层次_8063"><xsl:value-of select="$agr - 1"/></xsl:attribute>
			<xsl:attribute name="组合列表_8064"><xsl:for-each select="xdr:sp"><xsl:variable name="id"><xsl:if test="xdr:nvSpPr"><xsl:value-of select="xdr:nvSpPr/xdr:cNvPr/@id"/></xsl:if><xsl:if test="xdr:nvCxnSpPr"><xsl:value-of select="xdr:nvCxnSpPr/xdr:cNvPr/@id"/></xsl:if></xsl:variable><xsl:value-of select="concat($s_name,'_OBJ0000',$id,' ')"/></xsl:for-each><xsl:if test="xdr:pic"><xsl:for-each select="xdr:pic"><xsl:variable name="id"><xsl:value-of select="xdr:nvPicPr/xdr:cNvPr/@id"/></xsl:variable><xsl:value-of select="concat($s_name,'_OBJ0000',$id,' ')"/></xsl:for-each></xsl:if><xsl:if test="xdr:grpSp"><xsl:for-each select="xdr:grpSp"><xsl:variable name="id" select="xdr:nvGrpSpPr/xdr:cNvPr/@id"/><xsl:value-of select="concat($s_name,'_OBJ0000',$id)"/></xsl:for-each></xsl:if></xsl:attribute>
	...
	<xsl:template match="xdr:grpSpPr" mode="second">
		...
		<xsl:if test="a:xfrm/a:ext">
			<图:大小_8060>
				<xsl:attribute name="长_C604"><xsl:value-of select="a:xfrm/a:ext/@cy div 12700"/></xsl:attribute>
				<xsl:attribute name="宽_C605"><xsl:value-of select="a:xfrm/a:ext/@cx div 12700"/></xsl:attribute>
			</图:大小_8060>
		</xsl:if>
		<xsl:if test="a:scene3d">
			<图:三维效果_8061>
		...
		<图:图形_8062>
			...
			<图:预定义图形_8018>
				...
				<图:属性_801D>
					...
					<图:大小_8060 长_C604="199.49696">
						<xsl:attribute name="宽_C605"><xsl:value-of select="$grapWidth"/></xsl:attribute>
					</图:大小_8060>
					<!-- end -->
				</图:属性_801D>
			</图:预定义图形_8018>
		...
		<图:阴影_8051 是否显示阴影_C61C="true" 类型_C61D="single">
			<xsl:choose>
				<xsl:when test="a:srgbClr">
					<xsl:attribute name="颜色_C61E"><xsl:if test="a:srgbClr/@val"><xsl:value-of select="concat('#',a:srgbClr/@val)"/></xsl:if><xsl:if test="not(a:srgbClr/@val)"><xsl:value-of select="'auto'"/></xsl:if></xsl:attribute>
				</xsl:when>
				<xsl:when test="a:prstClr">
					<xsl:attribute name="颜色_C61E"><xsl:apply-templates select="a:prstClr"/></xsl:attribute>
				</xsl:when>
				<xsl:when test="a:schemeClr">
					<xsl:attribute name="颜色_C61E"><xsl:apply-templates select="a:schemeClr"/></xsl:attribute>
				</xsl:when>
				<xsl:otherwise>
					<xsl:attribute name="颜色_C61E"><xsl:value-of select="'auto'"/></xsl:attribute>
				</xsl:otherwise>
			</xsl:choose>
			<xsl:if test="//a:alpha">
				<xsl:choose>
					...
					<xsl:when test="./@dir">
						<xsl:choose>
							<xsl:when test="./@dir &lt; '5400000'">
								<xsl:attribute name="x_C606"><xsl:value-of select="'6.0'"/></xsl:attribute>
								<xsl:attribute name="y_C607"><xsl:value-of select="'6.0'"/></xsl:attribute>
							</xsl:when>
							<xsl:when test="./@dir &gt; '5400000'">
								<xsl:attribute name="x_C606"><xsl:value-of select="'-6.0'"/></xsl:attribute>
								<xsl:attribute name="y_C607"><xsl:value-of select="'6.0'"/></xsl:attribute>
							...
							<xsl:otherwise>
								<xsl:attribute name="x_C606"><xsl:value-of select="'2.0'"/></xsl:attribute>
								<xsl:attribute name="y_C607"><xsl:value-of select="'2.0'"/></xsl:attribute>
							</xsl:otherwise>
						</xsl:choose>
					</xsl:when>
					<xsl:otherwise>
						<xsl:if test="x">
							<xsl:attribute name="x_C606"><xsl:value-of select="round(x)"/></xsl:attribute>
						</xsl:if>
						<xsl:if test="not(x)">
							<xsl:attribute name="x_C606"><xsl:value-of select="'-6.0'"/></xsl:attribute>
						</xsl:if>
						<xsl:if test="y">
							<xsl:attribute name="y_C607"><xsl:value-of select="round(y)"/></xsl:attribute>
						</xsl:if>
						<xsl:if test="not(y)">
							<xsl:attribute name="y_C607"><xsl:value-of select="'6.0'"/></xsl:attribute>
						</xsl:if>
	...
	<xsl:template match="a:innerShdw" mode="shadow">
		<图:阴影_8051 是否显示阴影_C61C="true" 类型_C61D="single">
			<xsl:choose>
				<xsl:when test="a:srgbClr">
					<xsl:attribute name="颜色_C61E"><xsl:value-of select="concat('#',a:srgbClr/@val)"/></xsl:attribute>
				</xsl:when>
				<xsl:when test="a:prstClr">
					<xsl:attribute name="颜色_C61E"><xsl:apply-templates select="a:prstClr"/></xsl:attribute>
				</xsl:when>
			...
			<uof:偏移量_C61B>
				<xsl:if test="(@dir div 60000) = 0  or not(@dir)">
					<xsl:attribute name="x_C606"><xsl:value-of select="6.0"/></xsl:attribute>
					<xsl:attribute name="y_C607"><xsl:value-of select="0.0"/></xsl:attribute>
				</xsl:if>
				...
				<xsl:if test="(@dir div 60000) = 90">
					<xsl:attribute name="x_C606"><xsl:value-of select="0.0"/></xsl:attribute>
					<xsl:attribute name="y_C607"><xsl:value-of select="6.0"/></xsl:attribute>
				</xsl:if>
		...
		<xsl:if test="not(../a:sp3d/@extrusionH)">
			...
			<xsl:for-each select="./a:camera">
				...
				<xsl:if test="not(./a:rot)">
					...
					<xsl:choose>
						<xsl:when test="./@prst='orthographicFront' and ../a:lightRig/@rig='threePt' and ../a:lightRig/@dir='t'">
							<!--无旋转，只有在设置了三维效果时会出现-->
