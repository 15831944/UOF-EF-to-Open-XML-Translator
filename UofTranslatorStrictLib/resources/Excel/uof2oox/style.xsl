...
<xsl:stylesheet version="1.0" 
  ...
  <xsl:template name="Border_Style">
	  ...
	  <border xmlns="http://purl.oclc.org/ooxml/spreadsheetml/main">
		  <!--end-->
  ...
  <xsl:template name="BorderStyleTransfer">
	...
	<!--zl 20150415-->
    <xsl:if test="$lineType = 'mediumDashed'">
      <xsl:value-of select="'mediumDashed'"/>
    </xsl:if>
    <!--zl 20150415-->
    ...
    <!--2014-6-9, update by Qihy, 默认字体不正确，将fontId取值由0改为1 start-->
		<xf numFmtId="0" fontId="1" fillId="0" borderId="0" xfId="0" xmlns="http://purl.oclc.org/ooxml/spreadsheetml/main">
			<alignment vertical="center"/>
		</xf>
    <!--2014-6-9 end-->
		...
		<xsl:for-each select="式样:式样集_990B/式样:单元格式样集_9915/式样:单元格式样_9916[not(@标识符_E7AC='DEFSTYLE')]">
			...
			<xf xmlns="http://purl.oclc.org/ooxml/spreadsheetml/main">
				<xsl:attribute name="numFmtId">
