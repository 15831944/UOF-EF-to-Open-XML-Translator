...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="PPr-commen">
    ...
    <xsl:choose>
      <xsl:when test="ancestor::p:txBody/a:bodyPr/a:normAutofit/@lnSpcReduction">
        <xsl:variable name="scalelnspc" select="substring-before(ancestor::p:txBody/a:bodyPr/a:normAutofit/@lnSpcReduction,'%') * 1000"/>
        <xsl:variable name="lnSpc">
          <xsl:choose>
            <xsl:when test="a:lnSpc">
              <xsl:if test="a:lnSpc/a:spcPct">
                <xsl:value-of select="substring-before(a:lnSpc/a:spcPct/@val,'%') * 1000"/>
              </xsl:if>
      ...
      <xsl:when test="a:lnSpc">
        <字:行距_417E>
          <xsl:if test="a:lnSpc/a:spcPct">
            ...
            <!--2013-11-5, tangjiang, 修改OOXML到UOF行距百分制处理 start -->
            <xsl:variable name="lnSpcVal" select="substring-before(a:lnSpc/a:spcPct/@val,'%') * 1000"/>
            <xsl:variable name="lnSpc">
              <xsl:value-of select="$lnSpcVal div 100000"/>
            </xsl:variable>
            <!--end-->

            <xsl:attribute name="类型_417F">multi-lines</xsl:attribute>
            <xsl:attribute name="值_4108">
              <xsl:value-of select="$lnSpc"/>
            </xsl:attribute>
          </xsl:if>
          <xsl:if test="a:lnSpc/a:spcPts">
        ...
        <xsl:if test="a:spcBef">
          <字:段前距_4181>
            ...
            <xsl:if test="a:spcBef/a:spcPct">
              ...
              <!--2013-11-5, tangjiang, 修改OOXML到UOF段前距百分制处理 start -->
              <xsl:variable name="spcPctVal" select="substring-before(a:spcBef/a:spcPct/@val,'%')* 1000"/>
              <xsl:variable name="spcPct">
                <xsl:value-of select="number($spcPctVal) div 100000"/>
              </xsl:variable>
              <字:相对值_4184>
                <xsl:value-of select="$spcPct"/>
              </字:相对值_4184>
              <!--
              <字:相对值_4184>
                <xsl:value-of select="number(a:spcBef/a:spcPct/@val) div 100000"/>
              </字:相对值_4184>
              -->
              <!--end-->
            ...
            <xsl:if test="a:spcBef/a:spcPts">
              <字:绝对值_4183>
                ...
                <xsl:value-of select="number(a:spcBef/a:spcPts/@val) div 100"/>
                <!--</xsl:attribute>-->
              </字:绝对值_4183>
            </xsl:if>
            <!--自动 无对应-->
        ...
        <xsl:if test="a:spcAft">
          <字:段后距_4185>
            <xsl:if test="a:spcAft/a:spcPct">
              ...
              <!--2013-11-5, tangjiang, 修改OOXML到UOF段前距百分制处理 start -->
              <xsl:variable name="spcPctVal" select="substring-before(a:spcAft/a:spcPct/@val,'%')* 1000"/>
              <xsl:variable name="spcPct">
                <xsl:value-of select="number($spcPctVal) div 100000"/>
              </xsl:variable>
              <字:相对值_4184>
                <xsl:value-of select="number($spcPct)"/>
              </字:相对值_4184>
              <!-- 
              <字:相对值_4184>
                <xsl:value-of select="number(a:spcAft/a:spcPct/@val) div 100000"/>
              </字:相对值_4184>
              -->
              <!--end-->
            ...
            <xsl:if test="a:spcAft/a:spcPts">
              <字:绝对值_4183>
                <!--<xsl:attribute name="字:值">-->
                <xsl:value-of select="number(a:spcAft/a:spcPts/@val) div 100"/>
                <!--</xsl:attribute>-->
              </字:绝对值_4183>
            </xsl:if>
            <!--自动 无对应-->
  ...
  <xsl:template name="rPr">
    ...
    <字:句属性_4158>
      ...
      <xsl:choose>
          ...
          <字:字体_4128>
            ...
            <xsl:choose>
              <xsl:when test="ancestor::p:txBody/a:bodyPr/a:normAutofit/@fontScale">
                ...
                <!--2013-11-4, tangjiang, 修改OOXML到UOF字体缩放比的转换 start -->
                <!--修改前
						  <xsl:variable name="fontScale" select="ancestor::p:txBody/a:bodyPr/a:normAutofit/@fontScale div 100000"/>
              -->
                <xsl:variable name="fontScaleStr" select="ancestor::p:txBody/a:bodyPr/a:normAutofit/@fontScale"/>
                <xsl:variable name="fontScale">
                  <xsl:if test="contains($fontScaleStr,'%')">
                    <xsl:value-of select="substring($fontScaleStr,1,string-length($fontScaleStr)-1) div 100"/>
                  </xsl:if>
                  <xsl:if test="not(contains($fontScaleStr,'%'))">
                    <xsl:value-of select="$fontScaleStr div 100000"/>
                  </xsl:if>
                </xsl:variable>
                <!--end 2013-11-4, tangjiang, 修改OOXML到UOF字体缩放比的转换-->
              ...
              <xsl:otherwise>
                <xsl:choose>
                  ...
                  <!--2014-03-09, pengxin, 修改Strict OOXML到UOF字号转换错误 start -->
                  <xsl:otherwise>
                    <xsl:attribute name="字号_412D">
                      <xsl:value-of select="$defaultFontSize"/>
                    </xsl:attribute>
                  </xsl:otherwise>
                  <!--2014-03-09, pengxin, 修改Strict OOXML到UOF字号转换错误 end -->
        ...
        <!--修改字体转换liqiuling 2013-4-19  start-->
      ...
      <xsl:if test="@baseline">
        ...
        <!--2013-11-5, tangjiang, Strict OOXML转到UOF上下标用百分号表示的处理 start -->
        <xsl:variable name="baselineStr" select="@baseline"/>
        <xsl:variable name="baseline">
          <xsl:choose>
            <xsl:when test="contains($baselineStr,'%')">
              <xsl:value-of select="substring($baselineStr,1,string-length($baselineStr)-1)"/>
            </xsl:when>
            <xsl:otherwise>
              <xsl:value-of select="$baselineStr"/>
            </xsl:otherwise>
          </xsl:choose>
        </xsl:variable>
        <!--end-->
  ...
  <!--2014-03-04, tangjiang, 修改Strict OOXML到UOF字体颜色的转换 start -->
  ...
  <!-- end 2014-03-04, tangjiang, 修改Strict OOXML到UOF字体颜色的转换-->

  <!--liuyangyang 2014-11-17 添加模版用以获取表格所在行数 start-->
  ...
  <!-- end liuyangyang 2014-11-17 添加模版用以获取表格所在行数 -->
