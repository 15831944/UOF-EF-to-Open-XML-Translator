...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="mc:AlternateContent" mode="ph">
    <uof:锚点_C644>
      ...
      <xsl:choose>
        <xsl:when test="./mc:Choice//p:spPr/a:xfrm">
          <xsl:apply-templates select="./mc:Choice//p:spPr/a:xfrm" mode="ph"/>
          <xsl:apply-templates select="./mc:Choice//p:nvSpPr/p:nvPr/p:ph"/>
        </xsl:when>
        <xsl:otherwise>
  ...
  <!--end-->
	...
	<!--liuyangyang 2014-12-11 添加对smartart锚点的转换-->
  ...
  <!--end-->
    ...
    <!--liuyangyang 2014-12-11 对smart中的图形添加锚点-->
	...
	<xsl:template name="rel">
		...
		<xsl:if test="name($ParentcSld)='p:sld'">
			<xsl:variable name="rootTar" select="//rel:Relationships[@id=$sldRel]/rel:Relationship[@Type='http://purl.oclc.org/ooxml/officeDocument/relationships/slideLayout']/@Target"/>
      ...
      <xsl:if test="name($rootEleSp/p:spPr/a:xfrm)=''">
        <xsl:apply-templates select="$rootEleSp" mode="rel"/>
			...
			<xsl:choose>
				...
				<xsl:when test="not($rootEleSp)">
					...
					<xsl:variable name="rootTar1" select="//rel:Relationships[@id=$rootID1]/rel:Relationship[@Type='http://purl.oclc.org/ooxml/officeDocument/relationships/slideMaster']/@Target"/>
		...
		<xsl:if test="name($ParentcSld)='p:sldLayout'">
			<xsl:variable name="rootTar" select="//rel:Relationships[@id=$sldRel]/rel:Relationship[@Type='http://purl.oclc.org/ooxml/officeDocument/relationships/slideMaster']/@Target"/>
