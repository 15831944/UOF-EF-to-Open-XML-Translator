...
<xsl:stylesheet version="1.0"
                  ...
                  <xsl:if test="$spcBefVal">
                    <字:段前距_4181>
                      ...
                      <xsl:if test="$spcBefVal/a:spcPct">
                        <xsl:variable name="spcPctVal" select="substring-before($spcBefVal/a:spcPct/@val,'%')* 1000"/>
                  ...
                  <xsl:if test="$spcAftVal">
                    <字:段后距_4185>
                      <xsl:if test="$spcAftVal/a:spcPct">
                        <xsl:variable name="spcPctVal" select="substring-before($spcAftVal/a:spcPct/@val,'%')* 1000"/>
                        <xsl:variable name="spcPct">
                          <xsl:value-of select="number($spcPctVal) div 100000"/>
                        </xsl:variable>
                        <字:相对值_4184>
                  ...
                  <xsl:if test="$spcBefVal">
                    <字:段前距_4181>
                      ...
                      <xsl:if test="$spcBefVal/a:spcPts">
                        <字:绝对值_4183>
                          <!--<xsl:attribute name="字:值">-->
                          <xsl:value-of select="number($spcBefVal/a:spcPts/@val) div 100"/>
                          <!--</xsl:attribute>-->
                        </字:绝对值_4183>
                      </xsl:if>
                      <!--自动 无对应-->
                  ...
                  <xsl:if test="$spcAftVal">
                    <字:段后距_4185>
                      ...
                      <xsl:if test="$spcAftVal/a:spcPts">
                        <字:绝对值_4183>
                          <!--<xsl:attribute name="字:值">-->
                          <xsl:value-of select="number($spcAftVal/a:spcPts/@val) div 100"/>
                          <!--</xsl:attribute>-->
                        </字:绝对值_4183>
                      </xsl:if>
                      <!--自动 无对应-->
  ...
  <xsl:template match="a:fld" mode="field">
    ...
    <!--2014.12.03 guoyongbin 修改批注转换后只在第一张幻灯片中出现-->
    <!--<xsl:if test="//p:cmLst/p:cm and not(ancestor::p:sldMaster)">
      <xsl:variable name="comment" select="ancestor::p:sld/@commentID"/>
      <xsl:if test="//p:cmLst[@id = $comment]/p:cm and not(ancestor::p:sldMaster)">
        <xsl:for-each select="//p:cmLst[@id = $comment]/p:cm">
