...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="colorChoice">
    <xsl:param name="phClr"/><!--liuyangyang phClr-->
    <xsl:choose>
  ...
  <!--2014-01-13, tangjiang,修改Strict OOXML到UOF文字表式样的转换,文本框渐变填充 start -->
  <!--2014-11-03,liuyangyang，修改主题色转换模版-->
  ...
  <!-- end 2014-11-03,liuyangyang-->
  <!--2014-11-03,liuyangyang 添加主题色转换节点的递归计算-->
  ...
  <!-- end 2014-11-03,liuyangyang-->
  <!--2014-01-13, tangjiang,添加Strict OOXML到UOF文字表式样的转换,文本框渐变填充 start -->
  ...
  <xsl:template match="a:gs" mode="first">
    <xsl:if test="position()=1">
      <xsl:value-of select="substring-before(@pos, '%')*1000"/>
    </xsl:if>
  </xsl:template>
  <!--最后一个a:gs的属性信息-->
  ...
  <xsl:template match="a:gradFill">
    ...
    <图:渐变_800D>
      ...
      <xsl:variable name="angle">
        <xsl:choose>
          ...
          <xsl:when test="a:lin">
            ...
            <!--2014-03-09, pengxin, 修改Strict OOXML到UOF母版-渐变颜色方向错误 start -->
      ...
      <xsl:choose>
        ...
        <!--2014-03-09, pengxin, 修改Strict OOXML到UOF母版-渐变颜色方向错误，互操作渐变反向错误 start -->
       ...
       <!-- end 2014-03-09, pengxin 修改Strict OOXML到UOF母版-渐变颜色方向错误，互操作渐变反向错误-->
          
          <xsl:for-each select="a:gsLst/a:gs">
            <xsl:sort select="substring-before(@pos, '%')" data-type="number"/>
            <xsl:if test="position()=1">
            ...
            <xsl:if test="position()=last()">
              <xsl:attribute name="起始色_800E">
                <xsl:call-template name="colorChoice">
                  <xsl:with-param name="phClr" select="$phClr"/><!--liuyangyang phClr-->
                </xsl:call-template>
              </xsl:attribute>
            </xsl:if>
          </xsl:for-each>
...
-->
  <!--2014.11.18,liuyangyang添加对颜色透明度的计算模版 start-->
  ...
  <!--2014.10.29,liuyangyang添加对rgb颜色调节hsl分量的计算模版-->
