		...
		<!--<演:母版 uof:locID="p0036" uof:attrList="标识符 名称 类型 页面设置引用 配色方案引用 页面版式引用 文本式样引用" 演:类型="slide" 演:页面设置引用="IDymsz">-->
			...
			<xsl:if test="p:clrMap/@*">
				<xsl:attribute name="配色方案引用_6BEB">
					<!--xsl:value-of select="substring-before(substring-after(following-sibling::rel:Relationships[@id=concat($masterID,'.rels')]/rel:Relationship[@Type='http://purl.oclc.org/ooxml/officeDocument/relationships/theme' or @Type='http://purl.oclc.org/ooxml/officeDocument/relationships/themeOverride']/@Target,'theme/'),'.xml')"/-->
