...
xmlns:字="http://schemas.uof.org/cn/2009/wordproc">
  ...
  <xsl:template name="presentation" match="/">
	...
	<!--		<xsl:if test=" not(//mc:AlternateContent[mc:Choice/p:sp/p:txBody/a:p/a14:m/m:oMathPara]/mc:Fallback/p:sp/p:spPr/a:blipFill/a:blip)">-->
      ...
      <!--liuyangyang 2015-02-03 修改ole对象丢失 start-->
      <xsl:if test="//w:ole">
        <xsl:copy-of select="//w:ole/*"/>
      </xsl:if>
      <!--end liuyangyang 2015-02-03 修改ole对象丢失-->
      <!--元数据-->
          ...
          <xsl:for-each select=".//p:cmLst">
            ...
            <xsl:for-each select=".//p:cm">
              <规则:批注_B66A>
                ...
                <!--2014.12.1 guoyongbin 批注只在第一第一张幻灯片显示，故为批注加区域引用编号-->
                ...
                <!--end 2014.12.1 guoyongbin 批注只在第一张幻灯片显示，故为批注加区域引用编号-->
    ...
    <xsl:variable name="imageType" select="'http://purl.oclc.org/ooxml/officeDocument/relationships/image'"/>
    <xsl:variable name="audioType" select="'http://purl.oclc.org/ooxml/officeDocument/relationships/audio'"/>
    <xsl:variable name="videoType" select="'http://purl.oclc.org/ooxml/officeDocument/relationships/video'"/>
  ...
  <xsl:template name="chartpic">
    ...
    <xsl:for-each select=".//rel:Relationship[@Type='http://purl.oclc.org/ooxml/officeDocument/relationships/chart']">
      <xsl:variable name="rel">
        <xsl:value-of select=".//rel:Relationship[@Type='http://purl.oclc.org/ooxml/officeDocument/relationships/chart']"/>
      </xsl:variable>
