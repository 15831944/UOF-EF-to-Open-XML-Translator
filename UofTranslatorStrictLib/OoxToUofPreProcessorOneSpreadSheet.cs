    ...
    class OoxToUofPreProcessorOneSpreadSheet : AbstractProcessor
    {
        ...
        public override bool transform()
        {
            ...
            try
            {
                ...
                XPathDocument doc = new XPathDocument(extractPath + @"\workbook.xml");
            ...
            string preTimeFmtToSerialFileName = extractPath + "timeFmt.xml";
            TimeFmtToSerial(preOutPutFileName, preTimeFmtToSerialFileName);
           ...
           // NormalToRC(dataValidationPre, normalToRCFileName);
            NormalToRC(dataValidationPre, outputFile);
            //add talbe styles
            //AddTablesStyle(extractPath, normalToRCFileName, outputFile);

            //DataValidationPretreatment(preTimeFmtToSerialFileName, outputFile);
            return isSuccess;

        }


        #region TimeFormatToSerial
        ...
        private void TimeFmtToSerial(string input, string output)
        {
            ...
            {
                XmlNodeList cs = worksheet.SelectNodes("ws:sheetData//ws:c[@t=\"d\"]", nm);
                foreach (XmlNode c in cs)
                {
                    double serialC = 0.000;
                    XmlNode value = c.SelectSingleNode("ws:v", nm);
                    string timeValue = value.InnerText;
                    if (timeValue.Contains("T"))
                    {
                        string date = timeValue.Substring(0, timeValue.IndexOf("T"));
                        string time = timeValue.Substring(timeValue.IndexOf("T") + 1);
                        serialC = ConvertDateToSerial(date) + ConvertTimeToSerial(time);
                    }
                    else if (timeValue.Contains(":"))
                    {
                        serialC = ConvertTimeToSerial(timeValue);
                    }
                    else
                    {
                        serialC = ConvertDateToSerial(timeValue);
                    }

                    value.InnerText = serialC.ToString();
                }
            }
            xdoc.Save(output);
        }
        ...
        /// <summary>
        /// 基准时间为1900年，计算距今时间的序列化时间，86400为序列化常数
        /// </summary>
        /// <param name="date">需计算的日期</param>
        /// <returns>序列化数</returns>
        private double ConvertDateToSerial(string date)
        {
            DateTime d = Convert.ToDateTime(date);
            DateTime baseDate = Convert.ToDateTime("1899-12-30");
            return (d - baseDate).TotalSeconds / 86400;
        }

        /// <summary>
        /// 序列化时间，86400为序列化常数
        /// </summary>
        /// <param name="time">00：00：00格式的时间</param>
        /// <returns>返回序列化数</returns>
        private double ConvertTimeToSerial(string time)
        {
            DateTime t = Convert.ToDateTime(time);
            double totalSeconds = t.Hour * 3600 + t.Minute * 60 + t.Second;
        ...
        private string[] GetRelComment(string sheetRel)
        {
            ...
            XmlNode commentTargetNode = xdoc.SelectSingleNode("pr:Relationships/pr:Relationship[@Type='http://purl.oclc.org/ooxml/officeDocument/relationships/comments']", nm);
            ...
            if (commentTargetNode != null)
            {
                ...
                XmlNode drawingNode = xdoc.SelectSingleNode("pr:Relationships/pr:Relationship[@Type='http://schemas.openxmlformats.org/officeDocument/2006/relationships/vmlDrawing']", nm);

                if (drawingNode != null)
                {
                    string vmlTarget = drawingNode.Attributes["Target"].Value;
        ...
        //2014-4-15 end
        /*
  private static void GetPosition(string path)
  {
      XmlDocument drawFile = new XmlDocument();
      drawFile.Load(path);
      XmlNameTable nTable = drawFile.NameTable;
      XmlNamespaceManager nm = new XmlNamespaceManager(nTable);
      nm.AddNamespace("ws", TranslatorConstants.XMLNS_WS);
      XmlNodeList drawNodesList = drawFile.SelectNodes("//xdr:wsDr/xdr:twoCellAnchor", nm);

      foreach (XmlNode drawNode in drawNodesList)
      {
      ...
      foreach (FileInfo drawing in dir.GetFiles())
      {
          if (drawing.Extension.ToLower().Equals(".xml"))
          {
              ...
              if (relsFile.Exists)
              {
                  ...
                  XmlNodeList r = rels.SelectNodes("//r:Relationship[@Type='http://purl.oclc.org/ooxml/officeDocument/relationships/chart']/@Target", nm);
                ...
                foreach (string chartRelFile in chartRelFiles)
                {
                    ...
                    if (File.Exists(fileName))
                    {
                        ...
                        try
                        {
                            ...
                            nmRel.AddNamespace("pr", TranslatorConstants.XMLNS_PR);

                            XmlNode pictNode = chartFileDoc.CreateNode(XmlNodeType.Element, "ws", "picture", TranslatorConstants.XMLNS_WS);
        ...
        private void DataValidationPretreatment(string input, string output)
        {
            ...
            nm.AddNamespace("ws", TranslatorConstants.XMLNS_WS);
            XmlNodeList roots = xmlDoc.SelectNodes("//..//ws:dataValidations", nm);
            XmlNodeList originalDataValidation = null;
            try
            {
                foreach (XmlNode dataValidations in roots)
                {
                    originalDataValidation = dataValidations.SelectNodes("ws:dataValidation", nm);
                    foreach (XmlNode node in originalDataValidation)
                    {
                        if (node.Attributes.GetNamedItem("sqref") != null)
                        {
                            ...
                            if (s2.Length > 1)
                            {
                                ...
                                {
                                    XmlNode xn = xmlDoc.CreateNode(XmlNodeType.Element, "ws:dataValidation", TranslatorConstants.XMLNS_WS);
            ...
            {
                logger.Error("Fail in OoxToUofPreProcessorOneSpreadSheet: " + ex.Message);
                logger.Error(ex.StackTrace);
            }
            finally
            {
        ...
        {
            nmgr.AddNamespace("c", TranslatorConstants.XMLNS_C);
        ...
        private static void addHyperlinks(String path)
        {
            ...
            foreach (String sheetFileName in sheetFileNames)
            {
                ...
                sheetDoc.Load(sheetFileName);
                XmlNamespaceManager nsmgr = new XmlNamespaceManager(sheetDoc.NameTable);
                nsmgr.AddNamespace("s", TranslatorConstants.XMLNS_WS);
                nsmgr.AddNamespace("r", TranslatorConstants.XMLNS_R);
        ...
        private static void AddxiaoGroup(string path)
        {
            ...
            foreach (String sheetFileName in sheetFileNames)
            {
                ...
                nsmgr.AddNamespace("s", TranslatorConstants.XMLNS_WS);
                nsmgr.AddNamespace("r", TranslatorConstants.XMLNS_R);
                nsmgr.AddNamespace("ws", TranslatorConstants.XMLNS_WS);
